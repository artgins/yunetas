...
SDATA (DTP_STRING,  "token_endpoint",   0,      "",     "Explicit OAuth2 token endpoint URL. Skips discovery when set together with --end-session-endpoint"),
SDATA (DTP_STRING,  "end_session_endpoint",0,   "",     "Explicit OIDC end_session endpoint URL. Skips discovery when set together with --token-endpoint"),
SDATA (DTP_STRING,  "client_id",        0,      "",     "OAuth2 client_id (Keycloak/Auth0/Azure AD/...)"),
...
SDATA (DTP_STRING,  "mqtt_connect_properties",0, "",    "MQTT CONNECT properties as JSON string (MQTT v5 only). E.g. '{\"receive-maximum\":100}'. session-expiry-interval is always set from mqtt_session_expiry_interval."),
...
PRIVATE int mt_start(hgobj gobj)
{
    ...
    *------------------------------------------------------------------------*/
    ...
    BOOL has_idp = !empty_string(issuer) || !empty_string(token_endpoint);
...
PRIVATE int create_mqtt_connector(hgobj gobj)
{
    ...
    const char *mqtt_connect_properties = gobj_read_str_attr(gobj, "mqtt_connect_properties");
    ...
    json_t * jn_config_variables = json_pack("{s:s, s:s, s:s, s:s, s:s, s:s, s:s, s:s, s:s, s:s, s:s, s:s, s:s, s:s, s:s, s:s}",
        ...
        "__mqtt_connect_properties__", mqtt_connect_properties,

        "__mqtt_will_topic__", mqtt_will_topic,
...
{
    return create_gclass(C_MQTT_TUI);
}
