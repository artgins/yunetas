...
The package version is read from `../YUNETA_VERSION` (e.g., `YUNETA_VERSION=7.0.1`). The release number is set in each architecture wrapper script (e.g., `RELEASE="9"`). The final package name follows the pattern:

```
yuneta-agent-<version>-<release>-<arch>.deb
```

Example: `yuneta-agent-7.0.1-9-amd64.deb`
