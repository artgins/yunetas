 ...
 *          MAIN.C
 *
 *          Main of test_c_node_paged_nodes
 ...
 *                      Names
 ***************************************************************************/
#define APP_NAME        "test_c_node_paged_nodes"
#define APP_DOC         "Test the `nodes` command paging"
...
{                                                                   \n\
    ...
    'services': [                                                   \n\
        {                                                           \n\
            'name': 'test_paged_nodes',                             \n\
            'gclass': 'C_TEST_PAGED_NODES',                         \n\
...
/***************************************************************************
 *  The `nodes` command is SDF_AUTHZ_X and asks `read` before answering.
 *  With no checker installed the default one is C_AUTHZ's, which looks for
 *  an authz SERVICE this test yuno does not run — so every command would be
 *  refused with -403 and the paging would never be reached. This test is
 *  about the shape of the answer, not about who may ask for it.
 ***************************************************************************/
...
static int register_yuno_and_more(void)
{
     ...
     *  Register gclass
     *--------------------*/
    result += register_c_test_paged_nodes();
    ...
    set_expected_results(
        ...
        json_pack("[{s:s}, {s:s}, {s:s}, {s:s}, {s:s}, {s:s}, {s:s}, {s:s}, {s:s}, {s:s}, {s:s}, {s:s}, {s:s}]",
            ...
            "msg", "Creating topic",
            "msg", "Playing yuno",
            "msg", "All c_node paged nodes tests PASSED",
            "msg", "Exit to die",
            "msg", "Exit to die",
            "msg", "Pausing yuno",
