...
#   from a current SDK.
#
STAMP="${STAGE}/yuneta/development/yunetas/outputs/lib/yuneta_libc.stamp"
if [ ! -s "${STAMP}" ]; then
...
cat > "${STAGE}/etc/fail2ban/filter.d/yuneta-nginx-probe.conf" <<'EOF'
...
datepattern = {^LN-BEG}%%ExY(?P<_sep>[-/.])%%m(?P=_sep)%%d[T ]%%H:%%M:%%S(?:[.,]%%f)?(?:\s*%%z)?
              ...
              {^LN-BEG}
EOF
chmod 0644 "${STAGE}/etc/fail2ban/filter.d/yuneta-nginx-probe.conf"

cat > "${STAGE}/etc/fail2ban/jail.d/yuneta-nginx.conf" <<'EOF'
#
...
#   target -- they should already be off. A botnet that cannot log in still
#   fills sshd's MaxStartups slots with connections that never authenticate,
#   and past 10 of them the stock sshd drops new connections AT RANDOM: the
#   operator's and the deploy tools' included. Measured 2026-09-13: up to
#   1,464 drops an hour on one node; none with these two lines.
#
#   LoginGraceTime  a connection that has not authenticated in 20 s is
#                   closed (stock: 120 s), so its slot is freed sooner.
#   MaxStartups     start dropping at 50 unauthenticated connections, not
#                   10, and refuse them all at 200.
#
...
EOF
chmod 0644 "${STAGE}/usr/lib/systemd/system/yuneta-webserver.service"
...
# so this is the single place that reads it.
cat > "${STAGE}/yuneta/bin/yuneta-webserver" <<'EOF'
#!/bin/sh
#######################################################################
...
exit 0
EOF
chmod 0755 "${STAGE}/yuneta/bin/yuneta-webserver"
...
# /etc/init.d/yuneta_agent started the web server itself and let it daemonize,
# so on upgrade there is a master holding :80 and :443 that the unit knows
# nothing about. Ask it to finish first, then start the unit -- otherwise
# ExecStart dies with "Address already in use" and the node keeps serving from
# a process nobody owns, which is the bug this unit exists to end.
#
