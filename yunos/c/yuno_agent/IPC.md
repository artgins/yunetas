...
array. Declared at [`kernel/c/gobj-c/src/gobj.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.h):
...
Flags at [`gobj.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.h):
...
Example, the minimal gclass [`c_timer.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_timer.c) declaration (paraphrased):
...
Per-state transitions are declared with `ev_action_t` ([`gobj.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.h)):
...
is a named array of states (`states_t`, [`gobj.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.h)), terminated by
`{0,0}`.

Minimal example from [`c_timer.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_timer.c):
...
Macros at [`kwid.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/kwid.h):
...
The framework itself calls `KW_DECREF(kw)` at [`gobj.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c) when there is no
...
The workhorse. Entry at [`kernel/c/gobj-c/src/gobj.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c). Path:

1. Find `dst->current_state`.
2. Look up `event` in the state's `ev_action_list`
   ([`_find_event_action`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c#L939)).
3. If not found:
...
Quoting verbatim from [`kernel/c/gobj-c/src/gobj.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c):
...
Entry at [`gobj.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c). Loops over `publisher->dl_subscriptions` and calls
...
If there are no subscribers, [`gobj.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c) logs
...
Signatures at [`gobj.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.h):
...
Storage at [`gobj.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c):
...
Unknown keys at the top level produce a warning ([`gobj.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c)).
...
| End                | File                                              | Role                                                     |
|--------------------|---------------------------------------------------|----------------------------------------------------------|
| [`C_IEVENT_SRV`](#gclass-c-ievent-srv)     | [`kernel/c/root-linux/src/c_ievent_srv.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_ievent_srv.c)          | Listens, and receives connections from clients.              |
| [`C_IEVENT_CLI`](#gclass-c-ievent-cli)     | [`kernel/c/root-linux/src/c_ievent_cli.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_ievent_cli.c)          | Initiates, and connects to a remote `C_IEVENT_SRV`.          |
...
Serialized in [`kernel/c/root-linux/src/msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c):
...
does [`gbuf2json`](#gbuf2json) + [`kw_deserialize`](#kw_deserialize) ([`msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c)).
...
Documented in [`kernel/c/root-linux/src/msg_ievent.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.h). Top-level
shape:
...
- `__md_yuno__` — set by the responder via [`msg_iev_set_back_metadata()`](#msg_iev_set_back_metadata)
  ([`msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c)). Survives the round-trip.
- `__temp__` — **stripped at the yuno boundary** ([`msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c)).
...
Push/pop helpers: `msg_iev_push_stack` ([`msg_ievent.c:327`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c#L327)),
`msg_iev_get_stack`, [`msg_iev_pop_stack`](#msg_iev_pop_stack) ([`msg_ievent.c:419`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c#L419)).

`IEVENT_STACK_ID = "ievent_gate_stack"` constant at [`msg_ievent.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.h).

### 4.4 Identity card handshake

When `C_IEVENT_CLI` opens its WS connection, it sends `EV_IDENTITY_CARD`
(declared at [`msg_ievent.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.h), defined at [`msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c)) carrying:
...
- `jwt` — optional bearer for auth (also accepted from Cookie if empty,
  [`c_ievent_srv.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_ievent_srv.c))
- `user`, `host`, `pid`, `watcher_pid` — caller metadata

The server validates ([`c_ievent_srv.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_ievent_srv.c)):
...
([`c_ievent_srv.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_ievent_srv.c)) and replies with `EV_IDENTITY_CARD_ACK`. The
client unblocks from `ST_WAIT_IDENTITY_CARD_ACK` ([`c_ievent_cli.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_ievent_cli.h)).
...
`ac_on_message` in [`c_ievent_srv.c:933`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_ievent_srv.c#L933):

1. [`iev_create_from_gbuffer`](#iev_create_from_gbuffer) ([`msg_ievent.c:152`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c#L152)) deserializes the WS
   frame.
2. Inspect the latest `ievent_gate_stack` entry — `dst_yuno`,
   `dst_role`, `dst_service` ([`c_ievent_srv.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_ievent_srv.c)).
3. Validate the role/name.
4. [`gobj_find_service(iev_dst_service)`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c#L5076)
...
5. **Authorize the per-message service** (since 7.6.0). The
   ...
   [`YUNO_AUTH.md`](https://github.com/artgins/yunetas/blob/7.14.0/yunos/c/yuno_agent/YUNO_AUTH.md) §4.5.
...
[`gobj_find_service()`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c#L5159)
...
schema, handler function, description. See [`c_agent.c`](https://github.com/artgins/yunetas/blob/7.14.0/yunos/c/yuno_agent/src/c_agent.c) for a real
example, and the agent's own `YUNO_LIFECYCLE.md` for the table semantics.

### 5.2 `gobj_command` — the public entry point

Signature at [`gobj.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.h):
...
The constants are in [`msg_ievent.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.h) and [`msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c). Do not confuse
the two: `SDATACM` is a static declaration, and `EV_MT_*` are runtime events
that travel on the wire.

### 5.4 `msg_iev_build_response`

Defined at [`msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c). Every command handler returns one of these
...
are `EV_MT_STATS` / `EV_MT_STATS_ANSWER` ([`msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c)). Use
...
`C_TCP` and `C_TCP_S` in [`kernel/c/root-linux/src/c_tcp.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_tcp.c), [`c_tcp_s.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_tcp_s.c).
Headline events:
...
`CONNECTED`, `WAIT_HANDSHAKE` (if TLS), `IDLE`. See [`c_tcp.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_tcp.c).
...
the `child_tree_filter` attribute ([`c_tcp_s.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_tcp_s.c)).

### 6.3 HTTP server example: [`C_PROT_HTTP_SR`](#gclass-c-prot-http-sr) → `C_TCP_S`

[`c_prot_http_sr.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_prot_http_sr.c): builds a `ghttp_parser` (a wrapper around
...
`C_WEBSOCKET` ([`c_websocket.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_websocket.c)) handles the HTTP Upgrade handshake then
parses [RFC 6455](https://datatracker.ietf.org/doc/html/rfc6455) frames. Output `EV_ON_MESSAGE` carries either the text or
binary payload. The `iamServer` attribute ([`c_websocket.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_websocket.c))
...
([`c_tcp_s.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_tcp_s.c)):
...
socket via [`ytls_new_secure_filter()`](#ytls_new_secure_filter) ([`c_tcp.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_tcp.c)), and from then
...
In each yuno's config ([`c_yuno.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_yuno.c)):
...
runtime is JavaScript ([`kernel/js/gobj-js/src/c_ievent_cli.js`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/js/gobj-js/src/c_ievent_cli.js)) instead of
...
same code path as for C clients ([`c_ievent_srv.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_ievent_srv.c)).
...
[`do_disconnect`](https://github.com/artgins/yunetas/blob/7.14.0/modules/c/mqtt/src/c_prot_mqtt.c#L1580)) is documented there.
...
[`msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c). If you need persistent metadata across hops,
put it under `__md_iev__` or use the stack.

### 8.7 `msg_iev_build_webix` is the same as `msg_iev_build_response`

The old name lingers in comments ([`msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c)) and possibly in
some test fixtures. They are aliases. Use the new name in new code.

### 8.8 `__default_service__` resolution is case-insensitive

`gobj_find_service` lowercases ([`gobj.c:5076`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c#L5076)). `"My_Service"` and
...
| What                                          | Where                                                                  |
|-----------------------------------------------|------------------------------------------------------------------------|
| Event type declarations                       | [`kernel/c/gobj-c/src/gobj.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.h)                                   |
| `EVF_*` event flags                           | [`kernel/c/gobj-c/src/gobj.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.h)                                   |
| Minimal gclass example                        | [`kernel/c/root-linux/src/c_timer.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_timer.c)                                    |
| `gobj_send_event` dispatcher                  | [`kernel/c/gobj-c/src/gobj.c:7441`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c#L7441)                                 |
| State-before-action ("IMPORTANT HACK")        | [`kernel/c/gobj-c/src/gobj.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c)                                 |
| `gobj_publish_event`                          | [`kernel/c/gobj-c/src/gobj.c:8877`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c#L8877)                                 |
| `gobj_subscribe_event` config keys            | [`kernel/c/gobj-c/src/gobj.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.h)                                 |
| Subscription storage                          | [`kernel/c/gobj-c/src/gobj.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c)                        |
| `mt_inject_event` hook                        | [`kernel/c/gobj-c/src/gobj.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.h), [`gobj.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c)                        |
| `KW_DECREF` / `KW_INCREF`                     | [`kernel/c/gobj-c/src/kwid.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/kwid.h)                                    |
| `C_IEVENT_SRV` / `C_IEVENT_CLI`               | `kernel/c/root-linux/src/c_ievent_srv.{c,h}`, `c_ievent_cli.{c,h}`     |
| `__md_iev__` structure                        | [`kernel/c/root-linux/src/msg_ievent.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.h)                           |
| `IEVENT_STACK_ID`                             | [`kernel/c/root-linux/src/msg_ievent.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.h)                              |
| Stack push/pop                                | [`kernel/c/root-linux/src/msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c)                         |
| Wire frame pack/unpack                        | [`kernel/c/root-linux/src/msg_ievent.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c)                         |
| `msg_iev_build_response`                      | [`kernel/c/root-linux/src/msg_ievent.c:541`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/msg_ievent.c#L541)                             |
| Identity card validation                      | [`kernel/c/root-linux/src/c_ievent_srv.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_ievent_srv.c)                       |
| Service routing                               | [`kernel/c/root-linux/src/c_ievent_srv.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_ievent_srv.c)                      |
| `gobj_find_service` + special names           | [`kernel/c/gobj-c/src/gobj.c:5076`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c#L5076)                                 |
| `gobj_command` / `gobj_stats` public API      | [`kernel/c/gobj-c/src/gobj.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.h)                                 |
| HTTP server protocol                          | [`kernel/c/root-linux/src/c_prot_http_sr.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_prot_http_sr.c)                             |
| WebSocket protocol                            | [`kernel/c/root-linux/src/c_websocket.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_websocket.c)                                |
| TCP transport (states + TLS hookup)           | [`kernel/c/root-linux/src/c_tcp.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_tcp.c)                       |
| TCP server (`child_tree_filter`)              | [`kernel/c/root-linux/src/c_tcp_s.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_tcp_s.c)                        |
| TLS abstraction                               | [`kernel/c/ytls/src/ytls.h`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/ytls/src/ytls.h)                                      |
| `public_services` / `required_services`       | [`kernel/c/root-linux/src/c_yuno.c`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/root-linux/src/c_yuno.c)                             |
| JS-side `C_IEVENT_CLI`                        | [`kernel/js/gobj-js/src/c_ievent_cli.js`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/js/gobj-js/src/c_ievent_cli.js)                                |
