...
| **`test_c_tranger`** | [`C_TRANGER`](#gclass-c-tranger) commands: `topics`, `list-keys` (counts and time spans), `open-iterator` and its pages by `t` and by `tm`. |
...
| **`test_delete_key_propagation`** | `tranger2_delete_key()` and who hears it (rt_mem and rt_disk feeds, a real follower); a key directory that cannot be removed keeps its filtered iterators. |
...
| **`test_tr_dt_unknown`** | A topic on a filesystem whose `readdir()` gives no `d_type`: the keys are found with `stat()` in `keys/`, and the cache is whole. |
...
| **`test_tr_treedb_hook_rename`** | A hook renamed in the parent's schema: the child's `fkey` mark is derived, never read from disk, so the links load under the new name. |
