...
A driver gclass (`C_TEST_UDP_TX`) creates a `C_UDP_S` child and sends it three
...
3. `two`, to the same socket.

After 500 ms the socket must hold `one two`, in order. Then `C_UDP_S` is
stopped and the yuno dies; the memory check at the end catches a leaked send
event or gbuffer.

Up to 7.25.4 both failed:
