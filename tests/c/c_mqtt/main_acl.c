 ...
 *          MAIN_ACL.C
 *
 *          Self-contained MQTT broker ACL enforcement test.
 *
 *          Embedded C_AUTHZ + C_MQTT_BROKER (enable_acl=true). The C_ACL
 *          driver authors a group/client ACL model in the broker treedb and
 *          fires EV_MQTT_ACL_CHECK at the broker (the same path the
 *          c_prot_mqtt2 PUBLISH/SUBSCRIBE gates use), asserting allow/deny.
 *
 *          No network: the broker is exercised through its ACL event, so the
 *          test is deterministic. The input/top sides are empty IOGATEs,
 *          present only because the broker resolves them at start.
 *
 ...
 *                      Names
 ***************************************************************************/
#define APP_NAME        "test_mqtt_" "acl"
#define APP_DOC         "Self-contained MQTT broker ACL enforcement test"
...
/*
 *  Dedicated work_dir, wiped at start so the broker treedb is fresh
 *  every run (deterministic ACL data).
 */
#define WORK_DIR        "/tmp/test_mqtt_acl"
