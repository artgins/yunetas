...
/***************************************************************
 *              Static NSS-free getaddrinfo for fully-static builds
 ***************************************************************/
 ...
 */
static void yuneta_freeaddrinfo(struct addrinfo *res)
{
    while(res) {
    ...
    char nameservers[][YUNETA_NS_ADDRLEN], int *count, int maxns)
{
    *count = 0;
    ...
    while(fgets(line, sizeof(line), f) && *count < maxns) {
        ...
        *end = '\0';
        if(*p) {
            strncpy(nameservers[*count], p, YUNETA_NS_ADDRLEN - 1);
            nameservers[*count][YUNETA_NS_ADDRLEN - 1] = '\0';
            (*count)++;
        }
    }
    fclose(f);
 ...
 * Returns number of records found, -1 on parse error.
 */
...
/* Redirect all call sites in this file to the static implementations */
