/****************************************************************************
 *          MAIN_TEST9_BROWSER_CANCEL.C
 *
 *          Self-contained auth_bff test: browser aborts mid-KC-round-trip.
 *
 *          Differs from main_test1_login only in:
 *            - default service is C_TEST9_BROWSER_CANCEL
 *            - mock Keycloak is configured with latency_ms=500 so the
 *              deferred-response path opens a clean window for the
 *              driver to close its http_cl before the response arrives
 *            - __bff_side__ IOGate uses persistent_channels=true to
 *              keep the BFF instance alive when the test client
 *              disconnects (so result_token_response still runs and
 *              exercises the browser_alive gate)
 *
 ...
 *                      Names
 ***************************************************************************/
#define APP_NAME        "test_auth_bff_test9_browser_cancel"
#define APP_DOC         "Self-contained auth_bff test: browser aborts mid-KC-round-trip"
...
{                                                                   \n\
    ...
    'services': [                                                   \n\
        {                                                           \n\
            'name': 'c_test9_browser_cancel',                       \n\
            'gclass': 'C_TEST9_BROWSER_CANCEL',                     \n\
...
static int register_yuno_and_more(void)
{
    ...
    res += register_c_test9_browser_cancel();
