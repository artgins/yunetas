...
)
{
     ...
     *  Alloc memory
     *-------------------------------*/
    tr_mqtt_t *trq = GBMEM_MALLOC(sizeof(tr_mqtt_t));
    if(!trq) {
...
{
    dl_flush(&((tr_mqtt_t *)trq)->dl_q_msg, free_msg);
    GBMEM_FREE(trq);
}
 ...
 ***************************************************************************/
PRIVATE void trmq_set_first_rowid(tr_mqtt_t * trq, uint64_t first_rowid)
{
    hgobj gobj = 0;
...
)
{
     ...
     *  Alloc memory
     */
    mq_msg_t *msg = GBMEM_MALLOC(sizeof(mq_msg_t));
    if(!msg) {
...
{
    hgobj gobj = (hgobj)json_integer_value(json_object_get(tranger, "gobj"));
    tr_mqtt_t *trq = (tr_mqtt_t *)(uintptr_t)kw_get_int(gobj, list, "trq", 0, KW_REQUIRED);
...
PUBLIC int trmq_load(tr_mqtt_t * trq)
{
    ...
    uint64_t last_first_rowid = kw_get_int(
        gobj,
        trq->topic,
        "first_rowid",  // get from topic_var (trmq_set_first_rowid)
        0,
        0
    );
    if(last_first_rowid) {
    ...
    if(trq->first_rowid) {
        trmq_set_first_rowid(trq, trq->first_rowid);
    }
...
{
    trmq_set_hard_flag(msg, TRQ_MSG_PENDING, 0);
...
//PUBLIC BOOL trmq_msg_is_t_ms(mq_msg_t *msg)
//{
//    return (((mq_msg_t *)msg)->md_record.__system_flag__ & sf_t_ms)?true:FALSE;
//}
//PUBLIC BOOL trmq_msg_is_tm_ms(mq_msg_t *msg)
//{
//    return (((mq_msg_t *)msg)->md_record.__system_flag__ & sf_tm_ms)?true:FALSE;
//}
...
)
{
    ...
    json_t *__md__ = trmq_get_metadata(jn_message);
    if(__md__) {
        json_object_set(kw_response, __MD_TRQ__, __md__);
        trmq_set_metadata(kw_response, "result", json_integer(result));
    } else {
        gobj_log_error(gobj, LOG_OPT_TRACE_STACK,
            "function",     "%s", __FUNCTION__,
            "msgset",       "%s", MSGSET_INTERNAL_ERROR,
