...
| `helpers` | `helpers.c` string helpers, the directory walks and listings (and the errno of a failed `mkrdir()`), the agent's audit record builder, the rotatory (log files, `exit_on_fail` only at the open, the size limit in bytes) |
...
| `c_tranger` | `C_TRANGER` commands: topics, keys, paged records; `add-record`; the `rt_id` of a live list's pushes; a handle refused to another session; the permission `mark-tm-order` and `add-record` ask |
