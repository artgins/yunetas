...
static int register_yuno_and_more(void)
{
    ...
    /*------------------------------*
     *  Expect exactly one warning:
     *  the C_AUTH_BFF idp_url+realm deprecation notice fired
     *  during mt_create.  Any other warning or error fails the test.
     *  kw_match_simple is a subset match — only the `msg` field is
     *  required to match, so the other fields ("function", "idp_url",
     *  "realm", …) do not need to be enumerated here.
     *------------------------------*/
    json_t *expected_warnings = json_pack("[{s:s}]",
        "msg", "👤BFF idp_url+realm are deprecated; use 'issuer' or explicit endpoints"
    );
