...
PRIVATE int do_handshake(hsskt sskt_)
{
    ...
    if(sskt->ytls->trace) {
        gobj_trace_msg(gobj, "------- do_handshake");
    }
    ...
    if(ret <= 0)  {
        ...
        */
        ...
        case SSL_ERROR_WANT_WRITE:
            if(sskt->ytls->trace) {
                gobj_trace_msg(gobj, "------- encrypt_data: %s",
...
PRIVATE int flush_encrypted_data(sskt_t *sskt)
{
    ...
    if(sskt->ytls->trace) {
        gobj_trace_msg(gobj, "------- flush_encrypted_data()");
    }
    /*
    BIO_read() return
    ...
    */
    ...
    while((pending = BIO_pending(sskt->wbio))>0) {
        ...
        if(sskt->ytls->trace) {
            gobj_trace_msg(gobj, "------- flush_encrypted_data() %d", ret);
        }
        if(ret > 0) {
...
)
{
    ...
    while(sskt->ssl && (len = gbuffer_chunk(gbuf))>0) {
        ...
        if(sskt->ytls->trace) {
            gobj_trace_dump(gobj, p, len, "------- ==> encrypt_data DATA");
        }
        gbuffer_get(gbuf, written);    // Pop data
...
PRIVATE int flush_clear_data(sskt_t *sskt)
{
    ...
    if(sskt->ytls->trace) {
        gobj_trace_msg(gobj, "------- flush_clear_data()");
    }
    while(sskt->ssl) {
        ...
        if(sskt->ytls->trace) {
            gobj_trace_msg(gobj, "------- flush_clear_data() %d", nread);
        }
        if(nread <= 0) {
            sskt->error = ERR_get_error();
            if(sskt->error < 0) {
