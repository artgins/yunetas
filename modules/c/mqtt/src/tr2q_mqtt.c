 ...
 ***************************************************************************/
PRIVATE BOOL queue_topic_file_path(char *path, size_t size, tr2_queue_t *trq, const char *filename)
{
 ...
 ***************************************************************************/
PRIVATE void stat_queue_topic_file(tr2_queue_t *trq, const char *filename, struct stat *st)
{
    char path[PATH_MAX];
 ...
 ***************************************************************************/
PRIVATE int queue_topic_desc_loads(tr2_queue_t *trq)
{
    char path[PATH_MAX];
 ...
 ***************************************************************************/
PRIVATE BOOL queue_topic_keys_listable(tr2_queue_t *trq)
{
    char keys[PATH_MAX];
 ...
 ***************************************************************************/
PRIVATE int what_blocks_queue_topic(tr2_queue_t *trq)
{
 ...
 ***************************************************************************/
PRIVATE BOOL queue_topic_worth_opening(tr2_queue_t *trq)
{
    struct stat st;
...
PUBLIC int tr2q_unload_msg(q2_msg_t *msg, int32_t result)
{
    ...
    if(msg->inflight) {
        ret = dl_delete(&msg->trq->dl_inflight, msg, free_msg);
    } else {
        ret = dl_delete(&msg->trq->dl_queued, msg, free_msg);
    }
    return ret;
}
