...
Gobjs are modular components that form the building blocks of Yuneta applications. The creation and configuration of gobjs are flexible. This allows developers to instantiate individual gobjs or entire hierarchies using structured APIs and configurations.
...
- **[`gobj_create_tree0`](https://github.com/artgins/yunetas/blob/7.14.0/kernel/c/gobj-c/src/gobj.c#L1941)**: Creates a GObj tree based on a JSON structure (`jn_tree`).
...
| `gobj_flag_volatil`       | `0x0008`    | Marks the GObj as volatile. This means it is not persistent.                      |
...
GObj hierarchies can be defined using JSON structures. This makes it easy to specify attributes, children, and behavioral flags. A typical JSON tree structure includes:
...
| `as_service`      | Boolean indicating if the GObj must act as a service.                                          |
| `autostart`       | Boolean indicating if the GObj must start automatically.                                       |
| `autoplay`        | Boolean indicating if the GObj must play automatically.                                        |
| `disable`         | Boolean indicating if the GObj must be disabled.                                               |
...
- **Dynamic Behavior:** Flags and attributes provide fine-grained control over the GObj's function.
