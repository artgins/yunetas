...
**Source:** `tests/c/c_udp_s_tx/`, `tests/c/c_udp_s_restart/`, `tests/c/c_udp_s_self_stop/`, `tests/c/c_udp_s_rx/`, `tests/c/c_udp_s_echo/`
...
| **`c_mqtt/client_queues`** | An MQTT CLIENT with persistent queues against a raw broker: a QoS 2 message received again with DUP=1 replaces the copy waiting for its PUBREL and is delivered once, a later message with the same packet id is delivered as itself, and no *"QoS mismatch"* is logged; an outgoing message that expires while queued gives its slot to the next queued one. |
| **`c_mqtt/out_flight`** | A raw MQTT 5 client with Receive Maximum 2 against a broker with `max_inflight_messages` 0: two QoS 1 messages in flight, the others after the acks; `list-queues qos=1` lists only the pending message; a PUBCOMP of a QoS 1 message is a WARNING *"QoS mismatch"*, a DISCONNECT 0x82, and the message stays pending. |
...
| **`test_c_ievent_srv_peer_subs`** | What a remote peer may put in a subscription, and hold, through `C_IEVENT_SRV`: one peer writes its frames by hand, with a `__global__` that forges keys of the event and sets framework keys and a `gbuffer`, a `__local__` and a stray key; a plain client subscribes with a `__filter__` and a `__global__`; two local subscribers, one with a `__local__` and a `__global__`. Each gets what IT asked and nobody else's (up to 7.25.4 the publish shared one kw: the plain client got nothing, the later local subscriber got the forgery without the secret and the peer's `__md_iev__`, and the `gbuffer` crashed the yuno), and the publisher's kw comes back as it went. `max_subscriptions` and `max_subscription_size` refuse; the dropped keys, the caps, a withdrawal of nothing (repeated, with a 2 KB key: one line, capped) and a repeated authz refusal are each logged once; a client withdraws a subscription with a `__global__`; a `__username__` and routing stamps (`input_channel`, `input_service`) that the peer puts in a command, a stats request, an event or a subscription are overwritten by the gate's (up to 7.25.4 the peer's `__username__` reached the service); an event that carries a `gbuffer` reaches two remote subscribers (each on a twin with its own reference) and a local one, and when the publish returns the publisher holds the only reference, with no *"BAD gbuf_decref()"*. The stored subscription holds only the reversed hop of the peer's frame, not a 4 KB key of the peer's own in its `__md_iev__` (up to 7.25.4 the whole `__md_iev__`, not measured), and a routing bigger than `max_subscription_size` is refused, logged once; a subscription repeated 3 times is kept as it is (the publisher's `mt_subscription_added` is not called again), one warning; 3 commands to a service the channel may not reach and 2 that name no command get negative answers and one warning each; a frame with no `__md_iev__` (50 KB) is one capped warning and closes the channel. |
...
| **`test_tr_dt_unknown`** | A topic on a filesystem whose `readdir()` gives no `d_type`: the keys are found with `lstat()` in `keys/`, and the cache is whole. A key whose `lstat()` fails with `EIO` fails the listing, and the topic does not open (by `--wrap=lstat`). A symbolic link in `keys/` is not a key, with `d_type` or without it. A key directory of mode `r--` is flagged unreadable the same way without `d_type` as with it (up to 7.25.4, without `d_type`, it loaded EMPTY, with no log). |
...
| **`test_tr_queue_backup_failed`** | A backup that fails (the backup name taken by a file): `trq_check_backup()` / `tr2q_check_backup()` answer `-1`, the queue keeps its topic, and appends, reads and acks work after it; nothing leaks. And a backup whose new topic cannot be created after the move (its `mkdir` fails, `ENOSPC`): the backup is moved back, the queue keeps its topic whole, and the next call backs it up. A create that fails only at the `mkdir` of its `keys/` answers `NULL` and leaves nothing, and a backup that meets it keeps the queue's topic. A backup that fails and cannot open the topic again (`topic_desc.json` of mode `0` for a moment): the queue has no topic, a read and an ack fail (said once), and the queue takes its topic again by name as soon as it can be opened, and backs up at the next call (`tr_queue` and `tr2q`). While the topic still cannot be opened, the next calls log nothing. The same when what stopped the open is outside `topic_desc.json` (`keys/` of mode `0`): said once, and the topic is taken again once `keys/` can be listed, or as the tranger has it open again. And a tranger with `on_critical_error` `LOG_OPT_EXIT_ZERO` (the broker's queues): a backup whose create fails does not exit, and moves the backup back; a plain create whose `keys/` cannot be made exits (in a child process) only after it removed what it made, and the next create makes the topic whole. The CRITICAL *"Cannot create TimeRanger subdir. mkrdir() FAILED"* names the real cause (*"No space left on device"*, not *"Success"*). |
...
| **`test_tr2q_queued`** | The queued messages of an mqtt queue: one whose content cannot be read stays queued (`tr2q_move_from_queued_to_inflight()` answers `-1`), and `tr2q_check_backup()` does not back up while a message is queued. |
...
| **`test_c_treedb_literal_wins`** | [`C_TREEDB`](#gclass-c-treedb): a schema from C newer than the schema file in use wins whole; `__system__` is projected from it whole; the operator work it discards is reported once (`withdrawn_at_open`); an unfinished projection is recorded and completed at a later open, also after the process is killed at any write, once or twice. `save-schema` of a node of two parents (its place not written, `places_not_written`; unshared again by an edit of the other parent, it is no draft of the parent that remains), of two topics of one name (refused, `data.twins`), and of a draft that shifts its siblings (one save publishes them all, and `saved-schema`'s `draft_changed` names them first); an open that installs a newer literal and does not open keeps the saved schema. The scenarios are listed in its `README.md`. |
...
| **`test_audit_record`** | The audit record builder of `yuneta_agent`: secrets redacted, `content64` never written, the command word read as the parser reads it (`COMMAND=list-yunos` with a kw `command=delete-yuno` gets the full record), a secret inside a JSON text inside a JSON text redacted (up to 8 levels of escapes; deeper is written as its size and sha256), whatever quotes come before it or cut it (`x="{\"password\":…}"`, `'{\"password\":…}'`, `password=\"two words\"`), and the credentials after `Basic `; an escaped key with a blank or a slash (`\"secret key\":`), a quote inside a quoted secret (`password="a\" S"`, the shell's `'\''`), a JWT before a `.`, a write-attr whose names carry `\u` escapes or that comes as JSON text in a kw string; a deterministic fuzz of 20 000 generated commands (the secret in 24 quoting shapes among 17 kinds of noise, keys with blanks, slashes and `\u` escapes, write-attrs) writes no secret; hard shapes stay linear. The routing of a peer (`user`, each hop of `source`, `console_purpose`, the console name of `write-tty`) is redacted like any other string, and a field of it longer than 1024 bytes is written as its size and sha256. |
