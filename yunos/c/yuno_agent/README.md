...
| [`src/c_agent.h`](src/c_agent.h)                    | Public interface ([`register_c_agent`](https://github.com/artgins/yunetas/blob/7.15.0/yunos/c/yuno_agent/src/c_agent.c#L12063), `GOBJ_DECLARE_GCLASS`) |
...
- **What the `main()` of every yuno does** → [`ENTRY_POINT.md`](ENTRY_POINT.md)
  ...
  the gclass-registration callback). Then [`ydaemon.c`](https://github.com/artgins/yunetas/blob/7.15.0/kernel/c/root-linux/src/ydaemon.c): the double-fork
...
- **Auth + TLS** → [`YUNO_AUTH.md`](YUNO_AUTH.md) covers the `auth_bff` OIDC flow
  ...
  [`gobj_user_has_authz`](#gobj_user_has_authz) check at [`command_parser.c`](https://github.com/artgins/yunetas/blob/7.15.0/kernel/c/gobj-c/src/command_parser.c) is currently
...
- **The gobj framework in 30 minutes** → [`GOBJ.md`](GOBJ.md) explains the
  ...
  [`c_timer.c`](https://github.com/artgins/yunetas/blob/7.15.0/kernel/c/root-linux/src/c_timer.c) as the minimal canonical example.
