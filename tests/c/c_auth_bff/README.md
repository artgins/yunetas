...
| `test_auth_bff_test10_kc_silence` | **Deadlock reproducer**: Keycloak accepts the TCP connection but never responds. Mock KC `latency_ms=60000`, BFF `kc_timeout_ms=500`. Without the watchdog fix, the BFF would wedge until `auto_kill_time` expired; with it, the BFF fires `ac_kc_watchdog` at 500 ms, sends 504 to the browser via `send_error_response`, then drains the task through the normal `ac_end_task` cleanup. Asserts BFF stats `kc_timeouts=1, bff_errors=1, kc_ok=0` and mock KC `responses_sent=0, pending_cancelled=1`. |
