/****************************************************************************
 *          C_TEST14_METHOD_NOT_ALLOWED.C
 *
 *          Driver for test14_method_not_allowed: sends GET /auth/login
 *          instead of POST.  The BFF validates the method before anything
 *          else, so this test asserts:
 *            - HTTP 405 response
 *            - body.error_code == "method_not_allowed"
...
#include "c_test14_method_not_allowed.h"
...
PRIVATE int ac_on_open(hgobj gobj, gobj_event_t event, json_t *kw, hgobj src)
{
    ...
    if(gobj_trace_level(gobj) & TRACE_MESSAGES) {
        gobj_trace_msg(gobj, "test14: connected to BFF, sending GET /auth/login (expect 405)");
    }

    /*
     *  Send GET instead of POST — the BFF must reject with 405
     *  method_not_allowed before parsing the URL or body.
     */
...
PRIVATE int ac_on_message(hgobj gobj, gobj_event_t event, json_t *kw, hgobj src)
{
    ...
    if(gobj_trace_level(gobj) & TRACE_MESSAGES) {
        gobj_trace_msg(gobj, "test14: BFF response status=%d", status);
        if(jn_body) {
            gobj_trace_json(gobj, jn_body, "test14: BFF response body");
        }
    }
    ...
    if(status != 405) {
        gobj_log_error(gobj, 0,
            ...
            "msg",      "%s", "test14_method_not_allowed: expected BFF 405, got different status",
            "expected", "%d", 405,
            "status",   "%d", status,
            NULL
        );
    ...
    if(!jn_body || !json_is_object(jn_body)) {
        gobj_log_error(gobj, 0,
            "function", "%s", __FUNCTION__,
            "msgset",   "%s", MSGSET_APP_ERROR,
            "msg",      "%s", "test14_method_not_allowed: BFF error body missing or not an object",
            NULL
        );
        goto end;
    }
    if(json_is_true(json_object_get(jn_body, "success"))) {
        gobj_log_error(gobj, 0,
            "function", "%s", __FUNCTION__,
            "msgset",   "%s", MSGSET_APP_ERROR,
            "msg",      "%s", "test14_method_not_allowed: body.success must be false",
            NULL
        );
        goto end;
    }
    const char *got_code = kw_get_str(gobj, jn_body, "error_code", "", 0);
    if(strcmp(got_code, "method_not_allowed") != 0) {
        gobj_log_error(gobj, 0,
            "function", "%s", __FUNCTION__,
            "msgset",   "%s", MSGSET_APP_ERROR,
            "msg",      "%s", "test14_method_not_allowed: body.error_code mismatch",
            "expected", "%s", "method_not_allowed",
            "got",      "%s", got_code,
            NULL
        );
        goto end;
    ...
    if(empty_string(got_error)) {
        gobj_log_error(gobj, 0,
            "function", "%s", __FUNCTION__,
            "msgset",   "%s", MSGSET_APP_ERROR,
            "msg",      "%s", "test14_method_not_allowed: body.error fallback empty",
            NULL
        );
        goto end;
    }
    ...
    test_helpers_check_stats(gobj, bff, "test14_method_not_allowed", expected);
...
PRIVATE int ac_on_close(hgobj gobj, gobj_event_t event, json_t *kw, hgobj src)
{
    ...
    if(!priv->test_passed) {
        gobj_log_error(gobj, 0,
            "function", "%s", __FUNCTION__,
            "msgset",   "%s", MSGSET_APP_ERROR,
            "msg",      "%s", "test14_method_not_allowed: connection closed before success",
            NULL
        );
        TEST_HELPERS_BEGIN_DYING(priv);
    }
...
GOBJ_DEFINE_GCLASS(C_TEST14_METHOD_NOT_ALLOWED);
...
{
    return create_gclass(C_TEST14_METHOD_NOT_ALLOWED);
}
