/****************************************************************************
 *          MAIN_TEST13_REFRESH_EXPIRED.C
 *
 *          Self-contained auth_bff test: /auth/refresh against a Keycloak
 *          that rejects every /token request with 401 invalid_grant.
 *          Exercises the action-aware error mapping in c_auth_bff.c —
 *          the BFF must respond HTTP 401 to the browser with error_code
 *          "session_expired", NOT "invalid_credentials" (that code is
 *          reserved for the /auth/login path).
 *
 *          Same topology as main_test4_refresh, with the mock Keycloak's
 *          return_status overridden to 401 (same mock trick used by
 *          main_test2_kc_401 for the login case).
 *
 ...
 *                      Names
 ***************************************************************************/
#define APP_NAME        "test_auth_bff_test13_refresh_expired"
#define APP_DOC         "Self-contained auth_bff test: /auth/refresh with Keycloak returning 401 → session_expired"
...
{                                                                   \n\
    ...
    'services': [                                                   \n\
        {                                                           \n\
            'name': 'c_test13_refresh_expired',                     \n\
            'gclass': 'C_TEST13_REFRESH_EXPIRED',                   \n\
...
static int register_yuno_and_more(void)
{
    ...
    res += register_c_test13_refresh_expired();
     ...
     *  on a /auth/refresh call into a browser-facing HTTP 401 with
     *  error_code="session_expired" via send_error_response(), which by
     *  Yuneta convention also emits a gobj_log_error("BFF error response",
