 ...
 *          MAIN_OUT_FLIGHT.C
 *
 *          Self-contained MQTT broker test: with no maximum of its own
 *          (max_inflight_messages 0) the broker keeps the client's Receive
 *          Maximum in the out direction; `list-queues qos=` lists only the
 *          pending messages; and an ack of the wrong type is a protocol
 *          error of the peer (a WARNING and a DISCONNECT), not a delivery.
 *          See c_out_flight.c.
 *
 *          Embedded C_AUTHZ + C_MQTT_BROKER, an input gate on port 18114,
 ...
 *                      Names
 ***************************************************************************/
#define APP_NAME        "test_mqtt_" "out_flight"
#define APP_DOC         "Out in-flight window and acks of the broker"
...
static int register_yuno_and_more(void)
{
    ...
    set_expected_results(
        ...
        json_pack("[{s:s},{s:s}]",
            ...
            "msg", "QoS mismatch"   // the ack of the wrong type, said as a WARNING
        ),
