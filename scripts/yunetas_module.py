...
'''Tool for parsing a list of projects to determine if they are Yunetas
...
maintained in modules in addition to what is available in the main Yunetas tree.
'''
...
## metadata YAML file.
##
# The yunetas/module.yml file is a simple list of key value pairs to be used by
# the build system.
type: map
mapping:
  name:
...
MODULE_YML_PATH = PurePath('yunetas/module.yml')
# Path to the blobs folder
MODULE_BLOBS_PATH = PurePath('yunetas/blobs')
BLOB_PRESENT = 'A'
BLOB_NOT_PRESENT = 'D'
BLOB_OUTDATED = 'M'
...
def process_module(module):
    ...
    # The input is a module if yunetas/module.{yml,yaml} is a valid yaml file
    # or if both yunetas/CMakeLists.txt and yunetas/Kconfig are present.
    ...
    if Path(module_path.joinpath('yunetas/CMakeLists.txt')).is_file() and \
       Path(module_path.joinpath('yunetas/Kconfig')).is_file():
        return {'name': module_path.name,
                'name-sanitized': re.sub('[^a-zA-Z0-9]', '_', module_path.name),
                'build': {'cmake': 'yunetas', 'kconfig': 'yunetas/Kconfig'}}
...
def process_cmake(module, meta):
    ...
    module_path = PurePath(module)
    module_yml = module_path.joinpath('yunetas/module.yml')
    ...
    if cmake_extern:
        return('\"{}\":\"{}\":\"{}\"\n'
               .format(meta['name'],
                       ...
                       "${YUNETAS_" + meta['name-sanitized'].upper() + "_CMAKE_DIR}"))
    ...
    cmake_path = os.path.join(module, cmake_setting or 'yunetas')
...
def kconfig_snippet(meta, path, kconfig_file=None, blobs=False, sysbuild=False):
    ...
    snippet = [f'menu "{name} ({path.as_posix()})"',
	       ...
	       else f'osource "$(YUNETAS_{name_sanitized.upper()}_KCONFIG)"',
               f'config YUNETAS_{name_sanitized.upper()}_MODULE',
               '	bool',
               '	default y',
               'endmenu\n']
...
def process_kconfig(module, meta):
    ...
    kconfig_file = os.path.join(module, kconfig_setting or 'yunetas/Kconfig')
    if os.path.isfile(kconfig_file):
        return kconfig_snippet(meta, module_path, Path(kconfig_file),
                               blobs=taint_blobs)
    else:
        name_sanitized = meta['name-sanitized']
        return (f'config YUNETAS_{name_sanitized.upper()}_MODULE\n'
                f'   bool\n'
                f'   default y\n')
...
def process_meta(yunetas_base, west_projs, modules, extra_modules=None,
    ...
    yunetas_revision, yunetas_dirty = git_revision(yunetas_base)
    yunetas_project = {'path': yunetas_base,
                      'revision': yunetas_revision}
    meta['yunetas'] = yunetas_project
    meta['workspace'] = {}
    workspace_dirty |= yunetas_dirty
    ...
    if propagate_state:
        if workspace_dirty and not yunetas_dirty:
            yunetas_revision += '-dirty'
        if workspace_extra:
            yunetas_revision += '-extra'
        if workspace_off:
            yunetas_revision += '-off'
        yunetas_project.update({'revision': yunetas_revision})
...
def parse_modules(yunetas_base, manifest=None, west_projs=None, modules=None,
                  extra_modules=None):
    ...
    for project in modules + extra_modules:
        # Avoid including Yunetas base project as module.
        if project == yunetas_base:
            continue
        ...
        if meta:
            ...
            sys.exit(f'{project}, given in YUNETAS_EXTRA_MODULES, '
                     'is not a valid yunetas module')
...
def main():
    ...
    projects which are also a Yunetas module''', allow_abbrev=False)
    ...
    parser.add_argument('--meta-out',
                        help="""Write a build meta YaML file containing a list
                             of Yunetas modules and west projects.
    ...
    parser.add_argument('--meta-state-propagate', action='store_true',
                        help="""Propagate state of modules and west projects
                             to the suffix of the Yunetas SHA and if west is
    ...
    parser.add_argument('-z', '--yunetas-base',
                        help='Path to yunetas repository')
    ...
    modules = parse_modules(args.yunetas_base, None, west_projs,
...
#
# Modules may be set via YUNETAS_MODULES, YUNETAS_EXTRA_MODULES,
...
''')
    ...
    if args.meta_out:
        meta = process_meta(args.yunetas_base, west_projs, modules,
