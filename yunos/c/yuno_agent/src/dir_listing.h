 ...
 *  for an entry, a bad `match`) answers -1, with a comment that names the
 *  yuno and the directory, and no data: never an empty list, which reads as
 *  "the directory is empty". `kw` is owned, as by msg_iev_build_response().
 */
