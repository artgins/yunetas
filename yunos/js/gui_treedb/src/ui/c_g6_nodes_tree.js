...
SDATA(data_type_t.DTP_BOOLEAN,  "confirm_unlink_nodes", 0,  true,   "Ask confirmation before unlinking nodes"),
...
function request_unlink_edge(gobj)
{
    ...
    if(gobj_read_bool_attr(gobj, "confirm_unlink_nodes")) {
...
/************************************************************
 *  Show a confirmation popover next to the edge delete icon.
 ************************************************************/
function show_unlink_confirm(gobj, edgeData)
{
    ...
    // Position relative to the edge delete icon
    ...
    let msg = document.createElement('div');
    msg.style.cssText = 'margin-bottom:10px;font-weight:500;';
    msg.textContent = t('unlink') + ' ' + d.parent_id + ' → ' + d.child_id + '?';
    popover.appendChild(msg);
