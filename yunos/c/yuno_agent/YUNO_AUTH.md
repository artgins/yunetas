...
> ([`kernel/c/gobj-c/src/command_parser.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/command_parser.c)). It runs only when the yuno
...
Implemented in [`kernel/c/root-linux/src/c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c). URL dispatcher at
[`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c).
...
[`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c). The flow:
...
4. BFF validates `redirect_uri` against `allowed_redirect_uri`
   ([`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c), SEC-06).
5. BFF calls IdP `/token` with `grant_type=authorization_code` +
   `code_verifier` ([`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c)).
...
Built in [`make_set_cookie()`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c#L726) at [`c_auth_bff.c:726`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c#L726):
...
- `HttpOnly` ([`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c)) — JS cannot read.
- `Secure` — HTTPS only.
- `SameSite=Strict` — no cross-site CSRF.
- `Path=/` — sent with all requests to the origin.
- `Domain` ([`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c)) — set from `cookie_domain` attr, no port.
...
Logout clears both with `Max-Age=0` ([`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c)).

### 2.5 The OIDC config: `issuer` + (optional) explicit endpoints

`attrs_table` at [`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c):
...
- **HTTP_CL chain leak** ([`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c)). Under rapid
...
- **No real-IdP smoke tests.** The auth_bff test suite at
  `tests/c/c_auth_bff/` runs against [`c_mock_keycloak.c`](https://github.com/artgins/yunetas/blob/7.5.6/tests/c/c_auth_bff/c_mock_keycloak.c) only. Live
...
[`c_ievent_srv.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_ievent_srv.c) declares two volatile attributes the channel
exposes after auth:

- `http_cookie` — the raw cookie header (set by `c_authz` during upgrade).
- `jwt_payload` — the decoded JWT payload, also set by `c_authz`.

The comment at [`c_ievent_srv.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_ievent_srv.c) is explicit: *"HACK set by c_authz,
...
entry point is [`jwt_parse()`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/libjwt/src/jwt-verify.c#L85) in [`jwt-verify.c:85`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/libjwt/src/jwt-verify.c#L85). Keys come from [JWKS](https://datatracker.ietf.org/doc/html/rfc7517)
...
`client_id`. Per [`c_task_authenticate.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_task_authenticate.c):
...
After successful authn, [`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c) writes the resolved
...
The `C_AUTHZ` gclass ([`kernel/c/root-linux/src/c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c), 4114 lines)
...
`gobj_find_service_by_gclass(C_AUTHZ, TRUE)` ([`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c)).

### 4.1 The `authzs` treedb schema

[`kernel/c/root-linux/src/treedb_schema_authzs.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/treedb_schema_authzs.c). Three topics:
...
Roles can inherit from a parent (`parent_role_id`) — [`get_user_roles()`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c#L3367)
at [`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c) walks the chain and accumulates effective
authzs.

### 4.2 The `yuneta` super-user

[`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c):
...
to `"yuneta"` ([`c_agent.c`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c)) gives the agent itself this bypass for
its local CLI calls.
...
The predicate. [`gobj.h`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.h), body at [`gobj.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.c):
...
1. The gclass's own `mt_authz_checker` method, if declared
   ([`gobj.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.c)).
2. The globally-installed `__global_authorization_checker_fn__`
   ([`gobj.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.c)). This is set by `C_AUTHZ` at registration.
...
[`gobj.h`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.h). Two macros define the schema:
...
[`c_agent.c`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c):
...
[`command_parser.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/command_parser.c)
...
- **The gate — `enable_command_authz`.** A new `SDF_RD` boolean attr on
  ...
  [`c_yuno.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_yuno.c)).
...
- **External-only — the kw `__username__` marker.** The check fires **only for
  ...
  principal that [`c_ievent_srv`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_ievent_srv.c)
...
> test: [`tests/c/command_authz/test_command_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/tests/c/command_authz/test_command_authz.c)
...
[`gobj.h`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.h) declares the flags; [`gobj.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.c) declares the
...
2. **Custom code inside specific gclasses** that calls `gobj_user_has_authz`
   ...
   - Inside `auth_bff` for things like cookie-domain validation
     ([`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c) Host-vs-Domain matching, SEC-06).
...
[`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c) `attrs_table`):
...
| `authz.authz_yuno_role`   | **`SDF_DEPRECATED`**  | Legacy alias for `authz.authz_service`. Fallback at [`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c) — only read if `authz_service` is empty. New configs must use `authz.authz_service`. |
...
form is the lowercase `authz.*` used in [`yuno_agent/src/main.c`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/main.c).
...
empty. Both spellings reach [`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c) but the deprecated one
...
Declared in the `command_table` at [`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c). Just the names:
...
Agent-side: [`cmd_authzs_yuno`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c#L6210) ([`c_agent.c:6210`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c#L6210), registered as
`authzs-yuno` at [`c_agent.c`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c)) is the agent's wrapper to broadcast
...
[`c_agent.c`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c): when the post-snapshot diff says "changed",
[`cert_sync_broadcast_reload()`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c#L9386) sends `command=reload-certs service=__yuno__`
to every running yuno via [`cmd_command_yuno()`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c#L6090), plus the local agent.

Yunos without TLS listeners ignore the event. Yunos with TLS handle it
at [`c_tcp_s.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_tcp_s.c) — re-read the cert paths configured in their
...
[`cmd_cert_sync_now`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c#L6947) ([`c_agent.c:6947`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c#L6947)) forces a tick immediately.
[`cmd_cert_sync_status`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c#L6970) ([`c_agent.c:6970`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c#L6970)) returns the full state:
...
[`command_parser.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/command_parser.c). The most important thing in this document.
...
`EVF_AUTHZ_INJECT` and `EVF_AUTHZ_SUBSCRIBE` ([`gobj.h`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.h)) are
declared and the global authzs `__inject_event__` /
`__subscribe_event__` are registered ([`gobj.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.c)), but no check
...
([`gobj.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.c)). A yuno that did not register `C_AUTHZ` has zero
...
[`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c) permits the `yuneta` user to authenticate without
...
and constructs the URL automatically ([`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c)). Don't rely
on this — migrate the batches.

### 8.8 HTTP_CL chain leak on rapid disconnect

[`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c). During load testing with aggressive
...
`tests/c/c_auth_bff/` runs against [`c_mock_keycloak.c`](https://github.com/artgins/yunetas/blob/7.5.6/tests/c/c_auth_bff/c_mock_keycloak.c). Regressions
...
| What                                              | Where                                                                  |
|---------------------------------------------------|------------------------------------------------------------------------|
| `C_AUTH_BFF` gclass                               | [`kernel/c/root-linux/src/c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c)                                 |
| auth_bff yuno wrapper                             | [`yunos/c/auth_bff/src/c_auth_bff_yuno.c`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/auth_bff/src/c_auth_bff_yuno.c)                               |
| auth_bff endpoints dispatcher                     | [`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c)                                               |
| auth_bff attrs (`issuer`, deprecated `idp_url`)   | [`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c)                                                 |
| PKCE token call                                   | [`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c)                                               |
| Cookie builder                                    | [`c_auth_bff.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_auth_bff.c)                                                 |
| libjwt entry point                                | [`kernel/c/libjwt/src/jwt-verify.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/libjwt/src/jwt-verify.c)                                  |
| `C_AUTHZ` gclass                                  | [`kernel/c/root-linux/src/c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c)                                    |
| `authzs` treedb schema                            | [`kernel/c/root-linux/src/treedb_schema_authzs.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/treedb_schema_authzs.c)                |
| Role inheritance walk                             | [`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c) (`get_user_roles`)                               |
| `yuneta` super-user bypass                        | [`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c)                                                    |
| `__username__` write-side                         | [`c_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_authz.c)                                             |
| `gobj_user_has_authz`                             | [`gobj.h`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.h), [`gobj.c:9400`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.c#L9400)                                 |
| `SDATAPM` / `SDATAAUTHZ` macros                   | [`gobj.h`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.h)                                                       |
| Command authz check (gated opt-in)                | [`kernel/c/gobj-c/src/command_parser.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/command_parser.c)                          |
| `enable_command_authz` attr (`c_yuno`)            | [`kernel/c/root-linux/src/c_yuno.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_yuno.c)                              |
| Command authz regression test                     | [`tests/c/command_authz/test_command_authz.c`](https://github.com/artgins/yunetas/blob/7.5.6/tests/c/command_authz/test_command_authz.c) |
| `EVF_AUTHZ_*` flags                               | [`gobj.h`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/gobj-c/src/gobj.h)                                                       |
| Agent's cert_sync attrs                           | [`yunos/c/yuno_agent/src/c_agent.c`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c)                             |
| [`cert_sync_tick`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c#L9404) (diff + broadcast)               | [`c_agent.c`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c)                                                  |
| `cert_sync_broadcast_reload`                      | [`c_agent.c`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c)                                                  |
| `cert-sync-now` / `cert-sync-status` commands     | [`c_agent.c`](https://github.com/artgins/yunetas/blob/7.5.6/yunos/c/yuno_agent/src/c_agent.c)                                                  |
| `reload-certs` handler in TCP server              | [`kernel/c/root-linux/src/c_tcp_s.c`](https://github.com/artgins/yunetas/blob/7.5.6/kernel/c/root-linux/src/c_tcp_s.c)                            |
