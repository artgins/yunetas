...
| `test_unreadable_at_open` | A store damaged before the tranger starts: the key is flagged at the cache build, and every load of it says `load_failed`. A content that cannot be read ends the load. |
