...
# Writes/updates the yunetas/.config configuration file by merging configuration
# files passed as arguments, e.g. board *_defconfig and application prj.conf
# files.
#
# When fragments haven't changed, yunetas/.config is both the input and the
...
# Yunetas doesn't use tristate symbols. They're supported here just to make the
...
def main():
    ...
    if args.YUNETAS_BASE:
        os.environ['YUNETAS_BASE'] = args.YUNETAS_BASE
    ...
    if args.handwritten_input_configs:
        ...
        # loading yunetas/.config, because yunetas/.config is configuration
...
def missing_deps(sym):
    ...
    # even though Yunetas doesn't use them).
    #
...
SYM_INFO_HINT = """\
See http://docs.yunetasproject.org/latest/kconfig.html#CONFIG_{0.name} and/or
...
def parse_args():
    ...
    parser.add_argument("--forced-input-configs",
                        ...
                        help="Indicate the input configuration files are "
                             ...
                             " adjustments.")
    parser.add_argument("--yunetas-base",
                        help="Path to current Yunetas installation")
