 ...
 *          MAIN.C
 *
 *          Main of test_c_udp_s_echo: C_UDP_S reads and sends again
 *          of C_UDP_S goes back to its sender, whole
 *
 ...
 *                      Names
 ***************************************************************************/
#define APP_NAME        "test_c_udp_s_echo"
#define APP_DOC         "Test an answer in the rx gbuffer of C_UDP_S"
...
static int register_yuno_and_more(void)
{
     ...
     *  Register gclass
     *--------------------*/
    result += register_c_test_udp_echo();
    ...
    set_expected_results( // Check that no logs happen
        ...
        json_pack("[{s:s}, {s:s}, {s:s}, {s:s}, {s:s}, {s:s}, {s:s}]", // errors_list
            ...
            "msg", "UDP listening ...",
            "msg", "Playing yuno",
            "msg", "TEST: every answer in the rx gbuffer went back to its sender, whole",
            "msg", "Exit to die",
            "msg", "Pausing yuno",
...
int main(int argc, char *argv[])
{
    ...
    set_auto_kill_time(10);    // a stop that waits for ever (7.25.4) must not hang the suite
