...
(mt_create_resource)=
- [`mt_create_resource`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_resource2.c#L206):
   - Purpose: Creates a new resource managed by the GObject.
   - Example: Initializing data structures or external dependencies.

(mt_list_resource)=
- [`mt_list_resource`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_resource2.c#L425):
   - Purpose: Lists resources managed by the GObject.
   - Example: Returning an iterator or JSON representation.

(mt_save_resource)=
- [`mt_save_resource`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_resource2.c#L310):
   - Purpose: Saves or updates a resource.
   - Example: Persisting data to storage.

(mt_delete_resource)=
- [`mt_delete_resource`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_resource2.c#L359):
...
(mt_authenticate)=
- [`mt_authenticate`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_authz.c#L670):
...
(mt_disable)=
- [`mt_disable`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_channel.c#L118):
   - Purpose: Disables the GObject.
   - Example: Preventing further operations until re-enabled.

(mt_enable)=
- [`mt_enable`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_channel.c#L107):
...
(mt_create_node)=
- [`mt_create_node`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L798)
  - Purpose: Methods for managing nodes in TreeDB.
  - Example: Adding, updating, deleting, or linking nodes.

(mt_update_node)=
- [`mt_update_node`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L882)
  - Purpose: Methods for managing nodes in TreeDB.
  - Example: Adding, updating, deleting, or linking nodes.

(mt_delete_node)=
- [`mt_delete_node`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L1043)
  - Purpose: Methods for managing nodes in TreeDB.
  - Example: Adding, updating, deleting, or linking nodes.

(mt_link_nodes)=
- [`mt_link_nodes`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L1199)
  - Purpose: Methods for managing nodes in TreeDB.
  - Example: Adding, updating, deleting, or linking nodes.

(mt_unlink_nodes)=
- [`mt_unlink_nodes`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L1351):
   - Purpose: Methods for managing nodes in TreeDB.
   - Example: Adding, updating, deleting, or linking nodes.

(mt_topic_desc)=
- [`mt_topic_desc`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L628):
  - Purpose: Methods for inspecting TreeDB topics.
  - Example: Describing topics or retrieving metadata.

(mt_topic_links)=
- [`mt_topic_links`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L677):
  - Purpose: Methods for inspecting TreeDB topics.
  - Example: Describing topics or retrieving metadata.

(mt_topic_hooks)=
- [`mt_topic_hooks`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L737):
  - Purpose: Methods for inspecting TreeDB topics.
  - Example: Describing topics or retrieving metadata.
...
(mt_shoot_snap)=
- [`mt_shoot_snap`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L2026):
  - Purpose: Manage snapshots in the TreeDB.
  - Example: Creating or restoring data snapshots.

(mt_activate_snap)=
- [`mt_activate_snap`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L2049):
  - Purpose: Manage snapshots in the TreeDB.
  - Example: Creating or restoring data snapshots.

(mt_list_snaps)=
- [`mt_list_snaps`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L2070):
   - Purpose: Manage snapshots in the TreeDB.
   - Example: Creating or restoring data snapshots.
...
- **Read Operations**:
    ...
    - [`gobj_read_real_attr()`](#gobj_read_real_attr):
    - `gobj_read_str_attr()`:
    - [`gobj_read_json_attr()`](#gobj_read_json_attr):
...
- **Write Operations**:
    ...
    - [`gobj_write_real_attr()`](#gobj_write_real_attr):
    - [`gobj_write_json_attr()`](#gobj_write_json_attr):
    - [`gobj_write_new_json_attr()`](#gobj_write_new_json_attr):
    - [`gobj_write_pointer_attr()`](#gobj_write_pointer_attr):
...
- [`gobj_disable()`](#gobj_disable):
    - `mt_disable`: Disables the GObject.
- [`gobj_enable()`](#gobj_enable):
...
#### Resource Management
- [`gobj_create_resource()`](#gobj_create_resource):
    - `mt_create_resource`: Called when a resource is created.
- [`gobj_save_resource()`](#gobj_save_resource):
    - `mt_save_resource`: Called when a resource is saved.
- [`gobj_delete_resource()`](#gobj_delete_resource):
    - `mt_delete_resource`: Called when a resource is deleted.
- [`gobj_list_resource()`](#gobj_list_resource):
    - `mt_list_resource`: Called when a resource is list.
- [`gobj_get_resource()`](#gobj_get_resource):
    - [`mt_get_resource`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_resource2.c#L486): Called when a resource is get.
...
#### Authorization
- [`gobj_user_has_authz()`](#gobj_user_has_authz):
    - `mt_authz_checker`: Verifies if the user has the necessary permissions.

---

#### TreeDB Management
- [`gobj_treedbs()`](#gobj_treedbs):
    - `mt_treedbs`: Lists the TreeDBs associated with the GObject.
- [`gobj_treedb_topics()`](#gobj_treedb_topics):
    - [`mt_treedb_topics`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L609): Lists the topics within a TreeDB.
- [`gobj_topic_desc()`](#gobj_topic_desc):
    - `mt_topic_desc`: Describes a topic in the TreeDB.
- [`gobj_topic_links()`](#gobj_topic_links):
    - `mt_topic_links`: Lists the links for a topic.
- [`gobj_topic_hooks()`](#gobj_topic_hooks):
    - `mt_topic_hooks`: Lists the hooks for a topic.
- [`gobj_create_node()`](#gobj_create_node):
    - `mt_create_node`: Creates a new node in the TreeDB.
- [`gobj_update_node()`](#gobj_update_node):
    - `mt_update_node`: Updates an existing node in the TreeDB.
- [`gobj_delete_node()`](#gobj_delete_node):
    - `mt_delete_node`: Deletes a node from the TreeDB.
- [`gobj_get_node()`](#gobj_get_node):
    - [`mt_get_node`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L1498): Retrieves a specific node.
- [`gobj_list_nodes()`](#gobj_list_nodes):
    - [`mt_list_nodes`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L1548): Lists all nodes in a topic.
- [`gobj_list_instances()`](#gobj_list_instances):
    - [`mt_list_instances`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L1633): Lists instances of nodes.
- [`gobj_node_parents()`](#gobj_node_parents):
    - [`mt_node_parents`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L1699): Lists the parent nodes of a specific node.
- [`gobj_node_children()`](#gobj_node_children):
    - [`mt_node_children`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L1786): Lists the child nodes of a specific node.
- [`gobj_topic_jtree()`](#gobj_topic_jtree):
    - [`mt_topic_jtree`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L1880): Retrieves the JSON representation of a topic's tree structure.
- [`gobj_node_tree()`](#gobj_node_tree):
    - [`mt_node_tree`](https://github.com/artgins/yunetas/blob/7.17.3/kernel/c/root-linux/src/c_node.c#L1952): Retrieves the tree structure of nodes.

---

#### Snapshots
- [`gobj_shoot_snap()`](#gobj_shoot_snap):
    - `mt_shoot_snap`: Creates a snapshot.
- [`gobj_activate_snap()`](#gobj_activate_snap):
    - `mt_activate_snap`: Activates a snapshot.
- [`gobj_list_snaps()`](#gobj_list_snaps):
    - `mt_list_snaps`: Lists all snapshots.

---

#### Tracing
- [`gobj_set_gobj_trace()`](#gobj_set_gobj_trace):
...
- **Global Levels:**
  - [`gobj_get_global_trace_level()`](#gobj_get_global_trace_level): Get active global trace levels.
- **GClass Levels:**
...
- [`gobj_get_deep_tracing()`](#gobj_get_deep_tracing): Retrieve the current deep tracing level.
...
Production debugging starts and ends with [`gobj_log_error`](#gobj_log_error) and the
