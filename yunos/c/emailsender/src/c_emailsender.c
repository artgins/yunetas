...
typedef struct _PRIVATE_DATA {
    hgobj smtp;                 /* C_SMTP_SESSION pure_child (owns its C_TCP + reconnection) */
...
PRIVATE int tira_dela_cola(hgobj gobj)
{
    ...
    /*
     *  Don't touch the queue unless we're playing. gobj_pause() clears the
     *  playing flag BEFORE mt_pause()/close_queues() run, so this also keeps a
     *  deferred EV_ON_CLOSE (delivered during shutdown) from re-driving a
     *  closed queue — the real cause of the earlier shutdown crash. This is a
    ...
    /*
     *  Dispatch the head message to the SMTP child regardless of whether the
     *  link is currently up: connection handling is the CHILD's job. If its
     *  bottom C_TCP idle-closed (timeout_inactivity), c_smtp_session reconnects
     *  on demand when it receives this EV_SEND_MESSAGE (it must redo the SMTP
     *  handshake first) and sends once it reaches ST_IDLE. We never manage
     *  reconnections or timers up here.
     *
