...
The array holds the file NAMES, not full paths, in the order of the directory (not sorted: call `dir_array_sort()`). Only regular files are listed. A symbolic link is never listed, even a link to a regular file, and a directory is never listed. On a filesystem that gives no entry type (`DT_UNKNOWN`), the function uses `lstat()` so that the result is the same as with the entry type. Up to 7.25.4 that path used `stat()` and listed a link to a file. `tests/c/helpers/test_dir_read_error` checks that an `lstat()` of that path which fails with `EIO` fails the listing (-1, empty, logged).
