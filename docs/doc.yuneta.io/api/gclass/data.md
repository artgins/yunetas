...
| `set-impose-c-schema` | Show (no `set`) or change (`set=1` / `set=0`) `impose_c_schema`; with `treedb_name`, only that treedb (it leaves or enters `dynamic_schema_treedbs`). Needs the permission `impose-c-schema`. Acts the next time a treedb opens, and only in memory: a restart takes the configured value back. Its answer lists in `forced_by_code` the treedbs whose code imposes, which the value does not reach. |
