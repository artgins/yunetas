...
| **`test_c_treedb_literal_wins`** | [`C_TREEDB`](#gclass-c-treedb): a schema from C newer than the schema file in use wins whole; `__system__` is projected from it whole; the operator work it discards is reported once (`withdrawn_at_open`); an unfinished projection is recorded and completed at a later open, also after the process is killed at any write, once or twice. `save-schema` of a node of two parents (its place not written, `places_not_written`), of two topics of one name (refused, `data.twins`), and of a draft that shifts its siblings (one save publishes them all, and `saved-schema`'s `draft_changed` names them first); an open that installs a newer literal and does not open keeps the saved schema. The scenarios are listed in its `README.md`. |
