(tool-sync_configs)=
# [`sync_configs.py`](https://github.com/artgins/yunetas/blob/7.5.3/tools/agent/sync_configs.py)

Operator utility ([`tools/agent/sync_configs.py`](https://github.com/artgins/yunetas/blob/7.5.3/tools/agent/sync_configs.py)) that reconciles the yuno
...
- [`sync_binaries.py`](https://github.com/artgins/yunetas/blob/7.5.3/tools/agent/sync_binaries.py) drives from `$YUNETAS_BASE/outputs/yunos/` — every built
  binary lives in one well-known directory.
- Configs live scattered under each yuno's `batches/<host>/` directory, so
  [`sync_configs.py`](https://github.com/artgins/yunetas/blob/7.5.3/tools/agent/sync_configs.py) **drives from the current directory**. You `cd` into the
...
- **`version`** is *not* a parameter: the agent reads it from the `__version__`
  ...
  (See [`cmd_create_config`](https://github.com/artgins/yunetas/blob/7.5.3/yunos/c/yuno_agent/src/c_agent.c#L3744) / [`cmd_update_config`](https://github.com/artgins/yunetas/blob/7.5.3/yunos/c/yuno_agent/src/c_agent.c#L3904) in
