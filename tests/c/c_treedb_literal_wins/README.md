...
| LQ | `tw_lq` | The first open of a rowid projection (as LG) with a schema file in use and a topic an older release left: the ids move FIRST, so no warning, `departments` is no draft, and the record of the upgrade names it at its qualified ids; the next literal reports it `left_by_older_release`. Red before: a false warning, an empty record, and `departments` reported `"unsaved"`. |
