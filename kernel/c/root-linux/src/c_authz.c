...
/*  Reaction to an account created in an IdP (see c_idp_keycloak.c). */
PRIVATE int ac_idp_user_created(hgobj gobj, gobj_event_t event, json_t *kw, hgobj src);
...
/*---------------------------------------------*
 *      GClass authz levels
 *  The IdP-provisioning permissions moved with their commands to the
 *  provider gclass (c_idp_keycloak.c); nothing is enforced here (the
 *  legacy SDF_AUTHZ_X path in command_parser is disabled framework-wide).
 *---------------------------------------------*/
...
PRIVATE int mt_start(hgobj gobj)
{
    ...
    /*
     *  The seed of Authz.initial_load -- create what is missing, mark it
     *  immutable -- is not done here any more: it travels to the treedb's
     *  C_NODE as its `initial_load` attribute (see mt_create), and C_NODE
     *  applies it when it opens the treedb. It was the same loop written
     *  once for one treedb; every treedb needs it.
     */
...
/***************************************************************************
 *  Local method has_role — does this role id exist in treedb_authzs?
 *
 *  The roles belong to this plane, and an IdP provisioner has to be able to
 *  refuse an unknown one BEFORE it creates anything in the provider.  It is
 *  a local method and not a public C function because a gclass exposes
...
PRIVATE json_t *lmt_has_role(hgobj gobj, const char *lmethod, json_t *kw, hgobj src)
{
    ...
    if(!priv->gobj_treedb) {
        gobj_log_warning(gobj, 0,
            ...
            "msg",          "%s", "Treedb authzs not open, cannot check role",
            "role",         "%s", role,
            NULL
        );
    } else if(!empty_string(role)) {
...
/***************************************************************************
 *  An IdP provisioner created an account: record it in this plane.
 *
 *  This used to be written from inside the Keycloak pipeline, when that
 *  pipeline lived in this gclass.  It is now a reaction to an event, so
 *  every plane provisions itself and no provider has to know what a
 *  treedb_authzs user looks like.
 *
 *  The publisher reads the return: 0 means the user is recorded, and a
 *  negative return surfaces to the caller as a non-fatal warning.
 ***************************************************************************/
PRIVATE int ac_idp_user_created(hgobj gobj, gobj_event_t event, json_t *kw, hgobj src)
{
    PRIVATE_DATA *priv = gobj_priv_data(gobj);
    ...
    json_t *jn_add = json_pack("{s:s, s:b}", "username", username, "disabled", 0);
    if(!empty_string(role)) {
        json_object_set_new(jn_add, "role", json_sprintf("roles^%s^users", role));
    }
    gobj_send_event(gobj, EV_ADD_USER, jn_add, gobj);

    json_t *u = priv->gobj_treedb?
        gobj_get_node(priv->gobj_treedb, "users",
            json_pack("{s:s}", "id", username), 0, gobj) : NULL;
    BOOL created = u? TRUE : FALSE;
    JSON_DECREF(u)
    ...
    if(!created) {
        gobj_log_error(gobj, 0,
            "function",     "%s", __FUNCTION__,
            "msgset",       "%s", MSGSET_SERVICE,
            "msg",          "%s", "Cannot record the IdP user in treedb_authzs",
            "username",     "%s", username,
            NULL
        );
        KW_DECREF(kw)
...
GOBJ_DEFINE_EVENT(EV_IDP_USER_CREATED);

/*------------------------*
 *      Local methods
 *------------------------*/
PRIVATE LMETHOD lmt[] = {
    {"has_role",                lmt_has_role,           0},
    {0, 0, 0}
};
...
PRIVATE int create_gclass(gclass_name_t gclass_name)
{
    ...
    ev_action_t st_idle[] = {
        ...
        {EV_IDP_USER_CREATED,     ac_idp_user_created,      0},
