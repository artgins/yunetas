...
| GONE | `tw_l2` | A topic whose store directory is gone is not reported as `applied`. An apply is recorded when it is made (`saved_schemas/<treedb>.applied.json`), not guessed from a missing `topic_var.json`. |
...
| UNF | `tw_m1` | A projection is unfinished (a snapshot holds `departments`). `saved-schema` does not show the leftover as a draft. The operator adds a column to `users`: it is the only draft. The retry of the projection replaces that column and reports `users` as `unsaved`. A column added to the leftover topic makes that topic a draft: when the snapshot is gone, the projection completes and reports `departments` as `unsaved`. |
| LEFT | `tw_m2i`, `tw_m2d` | An unedited leftover of an unfinished projection is not reported as withdrawn work when the projection completes: on the imposed retry, and when a newer literal arrives. |
| SEED0 | `tw_l1` | A projection seeded from a dynamic file (`c_schema_version` 0) is not unfinished. The file itself, as the literal, projects nothing: the operator's column and header draft stay, nothing is reported. |
| APCR | `tw_l3` | The process died between the record of an apply and its rename (the record is written by hand). The next apply keeps the topics of the apply that ran, from the record's `previous`: a newer literal reports `users` as `in_use` and `departments` as `applied`. |
| SEEDF | `tw_l4` | A projection seeded from the file fails (a column flag that the meta-schema refuses). It is recorded, retried at every open, its partial projection is no draft, and `save-schema` refuses. When the file is fixed, the next open completes it. |
| FAIL | `tw_l6` | An open that fails answers `-1` and names the yuno: a literal that C_TREEDB refuses, and a schema file that `treedb_open_db()` refuses (no topics). The second answer tells to `close-treedb` first. |
| UNF2 | `tw_m1b` | A column the operator adds to the leftover topic stays a draft through a retry that cannot finish (the snapshot still holds the topic): `saved-schema` shows it, the record does not name it as a leftover, nothing is reported. The open that removes it reports `departments` as `unsaved`. |
| KEEP | `tw_m2r`, `tw_m2c` | A draft that the projection cannot replace (its delete is refused) stays a draft and is reported once, by the open that replaces it: a column and a header edit on a topic the literal removes (`tw_m2r`), and a column added to a topic the literal keeps (`tw_m2c`). |
| TORN | `tw_l1b` | The record of an unfinished projection is torn. It still says "unfinished": `saved-schema` answers `unfinished_projection`, `save-schema` refuses, ONE WARNING is logged at every read (with the cause in `error`, and no CRITICAL). The next open retries and writes the record again. What the torn record left is unknown, so it is taken for a draft and reported when the projection completes. |
| STAMP | `tw_l2b` | The node of a new treedb in `__system__` is created with `schema_version` and `c_schema_version` 0 (its first record on disk), and stamped last. |
| FAIL2, YUNO | `tw_l3b` | After an open that failed, a second open says that the treedb did not open and tells to `close-treedb`; the `treedbs` row has `opened: false`. `delete-treedb` is refused: it says that the treedb did not open, and to `close-treedb` it first. `close-treedb` logs nothing (C_NODE does not close a treedb that never opened). Writing `with_link_events` on the C_NODE of that treedb logs nothing either (there is no callback to change). Every answer of `open-treedb` and `close-treedb` starts with the yuno. |
| DEL | `tw_n7u`, `tw_n7s` | The operator deletes the topic `departments` in `__system__` (a draft). A newer literal declares it: the topic is created again, and the deletion is reported as `unsaved` (`tw_n7u`), or as `saved` with the withdrawn saved schema when `save-schema` published it (`tw_n7s`). |
| EMPTY | `tw_n1` | After `delete-treedb`, a seed that died before its end is emulated: the `treedbs` node with `schema_version` and `c_schema_version` 0, and no record. The next open completes it (the file runs), reports nothing, and stamps `c_schema_version` 1. Then the operator deletes every topic: `save-schema` refuses a draft with no topics, and a saved schema with no topics (written by hand) is refused: `saved-schema` answers `can_apply` false and says why, and `apply-schema` refuses it; the file in use does not change. |
| SAVRM | `tw_n4` | A SAVED draft on the topic that the literal removes, and a snapshot refuses the delete. The first open reports only the withdrawn saved schema, and the record keeps `draft_kinds: {"departments": "saved"}`. When the snapshot is gone, the open that completes the projection reports `departments` as `saved`. |
...
| DC | `tw_dcd_*`, `tw_dcn_*` | The process DIES TWICE: a child is killed at the k1-th write of the projection, a second child retries it and is killed at its first write or at its last (with drafts), or completes it, and then the parent opens twice; for every k1 (26 writes with drafts, 23 without), 99 sequences. Every process appends what it REPORTS (the warning of withdrawn work) to `<db>.reports` the moment it logs it, so a child that dies after its report is counted. Across the sequence the operator's drafts are reported EXACTLY ONCE, by whichever process completes the projection (often the second child), nothing that nobody did is reported, the parent's first open logs no error and warns only when it reports, its second open reports nothing, and no record is left. |
...
| DTK | `tw_dtk<k>` | `delete-treedb` dies at each of its writes of `__system__` (a child killed at write k), and runs again: the second run answers `0`, logs no error, and leaves no node of the treedb and no record. k 0 is a delete of 7.25.4 cut after its first write, the node of the treedb (deleted by hand): the nodes left in no tree are still the treedb's, and they go. |
| O4C | `tw_o4c<k\|r>_<n>_0` | 7.25.4 opening a literal that changes the headers of two columns of `users` dies after each of its writes (emulated write by write: the stamp first, then the topic, then its columns). The next open completes the projection, reports nothing, and the three homes agree; the next literal reports nothing. Once with the record of the upgrade kept, once removed. Red before: a topic with one column written and one not was reported `"unsaved"`. |
| O4A | `tw_o4a<k\|r>_<n>_0` | The same with a literal that adds a column to `users` and the topic `roles`: a create killed before its link leaves a node in no tree, which is the projection's (it is what the literal writes, and the old file does not declare it). Red before: `{"users": "unsaved"}` or `{"roles": "unsaved"}`. |
| O4X | `tw_o4<c\|a>r_<n>_<k>` | O4C and O4A on a 7.25.4 store, and the open that completes is killed too, at each of its writes k; the open after it completes. No process reports anything and no error is logged. Red before (without `stamped_base` in the record the retry compared with the old file alone). |
| FP | `tw_fp<mode>_<n>` | A FIRST projection of 7.25.4 over a schema file already at the literal's version (emulated: the treedb opened, its projection and records deleted, then 7.25.4's writes, the stamp first) died after each of its writes n. The first open by this release restores from the literal what `__system__` misses (mode 0: the file runs; 1: imposed) or takes it for no deletion (mode 2: a newer literal installed at that first open): nothing reported, no error, the three homes agree, and the next literal reports nothing. Then a topic the operator deletes (a draft) is NOT restored by the next open with the same literal: the record of the upgrade says this release has been here. Red before: never completed, every missing topic a draft, reported `"unsaved"` by the next literal. |
| OL | `tw_ol`, `tw_ole`, `tw_ol3` | What 7.25.4 LEFT in `__system__`: v2 dropped `departments` and `users.email`, and 7.25.4 never deleted them (emulated: v1 opened, the record of the upgrade removed, 7.25.4's writes and file v2). The first open with v2 shows no draft, withdraws and removes nothing, and warns nothing. v3 removes them and reports `{"departments": "left_by_older_release", "users": "left_by_older_release"}` with two warnings (the withdrawn one and the one naming the ids); v4 reports nothing. `tw_ole`: the operator edits `departments.name` after the upgrade, which makes it a draft (`draft_changed` `{"departments": true}`) reported `"unsaved"`. `tw_ol3`: the first open installs v3 and reports the same. Red before: `draft_changed` named both topics and v3 reported them `"unsaved"`. |
...
| COLF | `tw_l8` | The literal removes `departments`, which holds a column the operator added. The topic is deleted and the deletes of its columns fail: the draft is still in `__system__`, so it is not reported, and the record keeps its kind. The open that removes the columns reports it. |
...
| SH, SI | `tw_sh`, `tw_si` | 7.25.4 wrote the stamp, then each topic, then its columns. Its process died after it raised `users` to `topic_version` 2 and before it wrote the new header of `users.username`. The open with the literal 2 completes the projection: `__system__` shows the new header and holds no draft. The next literal reports nothing. `tw_sh` opens with the file, `tw_si` with `impose_c_schema`. |
| SJ | `tw_sj` | SE with `impose_c_schema`: the imposed open with the literal 2 completes the projection and reports nothing. |
| SK | `tw_sk` | With `impose_c_schema`, a header draft over a complete projection stays a draft when the same literal opens again, and nothing is reported. |
...
| OS | `tw_os`, `tw_ose` | What 7.25.4 left (as OL) is left out of `save-schema`: a save with no edit has nothing to save and names the four ids in `left_by_older_release`; a save after an edit of `users.username` publishes `users` (topic_version 3) without `email` and no `departments`, and the apply does not put them back. `tw_ose`: an edit of `departments.name` makes it the operator's, and the save publishes it. Red before: the save with no edit published both, and the apply put them back. |
| DF | `tw_dfa`, `tw_dfb` | The file in use holds its topics as a DICT. A seed from it after `delete-treedb` (`tw_dfa`), and the completion of an unfinished projection from it (`tw_dfb`), project every topic, remove the record, and show no draft. Red before: nothing was projected, the projection was stamped complete, and the missing topics read as drafts. |
| LQ | `tw_lq` | The first open of a rowid projection (as LG, its nodes what the projector of 7.13.1 wrote, field for field) with a schema file in use and a topic an older release left: the ids move FIRST, so no warning, NO draft (`users` included: its nodes, written before `order` existed, carry the default 9999, and that is no reorder), and the record of the upgrade names `departments` at its qualified ids; the next literal reports `departments` and `users` (its `operator_col`) as `left_by_older_release`. Red before: a false warning, an empty record, `departments` reported `"unsaved"`; and, after the move was fixed, `users` still a draft for its `order`, withdrawn `"unsaved"`. |
| LX | `tw_lx` | The move of a rowid projection fails half way (the keys directory of `cols` read-only), at an open that installs a newer literal: nothing is projected, the node keeps its old meta-schema version, `save-schema` refuses. With the directory writable, the next open completes the move. Red before: the projection of the same open stamped the meta-schema version, and no later open moved what was left. |
| SV | `tw_sv` | A literal with another content under the same `schema_version` as the file (a column added, the number not raised) is said with ONE WARNING at every open; the literal that IS the file says nothing. Red before: nothing was said when `__system__`'s `c_schema_version` was that same number. |
| SW | `tw_sw` | `save-schema` writes the saved schema whole: a new inode, no `.new` left; a save that cannot write (the directory read-only) answers `-1` and leaves the pending save. Red before: the file was rewritten in place (same inode). |
| GB | `tw_gb1`, `tw_gb2` | `saved-schema` with no `treedb_name` over two treedbs, its kw carrying a `gbuffer`: the caller keeps its reference, no error. Red before: *"BAD gbuf_decref()"* and the gbuffer freed. |
| EV | (none) | `EV_OPEN_TREEDB` and `EV_CLOSE_TREEDB`, public and not implemented, answer `-1` with ONE ERROR each. Red before: nothing was said. |
