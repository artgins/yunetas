...
- Delivery of posted events: every cycle of `yev_loop_run()` starts by calling `gobj_deliver_posted_events()`, so an action that used `gobj_post_event()` to leave the stack it was on continues on the next turn. While any are pending the loop does not block on the ring
