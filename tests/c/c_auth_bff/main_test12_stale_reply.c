/****************************************************************************
 *          MAIN_TEST12_STALE_REPLY.C
 *
 *          Self-contained auth_bff test: cross-user token leak race.
 *
 *          Same topology as main_test9_browser_cancel / test11 —
 *          persistent_channels=true on __bff_side__ so the second
 *          TCP connection reuses the same BFF instance, and mock KC
 *          latency_ms=500 so there's a window where A's reply is
 *          still pending when B's connection arrives.
 *
 ...
 *                      Names
 ***************************************************************************/
#define APP_NAME        "test_auth_bff_test12_stale_reply"
#define APP_DOC         "Self-contained auth_bff test: stale KC reply dropped by gen gate"
...
{                                                                   \n\
    ...
    'services': [                                                   \n\
        {                                                           \n\
            'name': 'c_test12_stale_reply',                         \n\
            'gclass': 'C_TEST12_STALE_REPLY',                       \n\
...
static int register_yuno_and_more(void)
{
    ...
    res += register_c_test12_stale_reply();
