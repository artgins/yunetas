 ...
 *          MAIN_MALFORMED.C
 *
 *          Self-contained MQTT broker malformed-frame regression test.
 *
 *          Embedded MQTT broker (C_AUTHZ + C_MQTT_BROKER) on a server port.
 *          The C_MALFORMED driver opens a RAW TCP client and injects a
 *          control packet whose MQTT "remaining length" is 0 (a malformed
 *          CONNECT, 0x10 0x00). Before the fix this left frame_completed()
 *          handing a NULL payload gbuffer to a handler that dereferenced it
 *          (gbuffer_leftbytes(NULL)) -> SIGSEGV, crashing the whole broker
 *          process from a single remote packet (see core.gate_mqtts).
 *
 *          After the fix the broker rejects the frame at the header stage,
 *          logging exactly one "Mqtt malformed packet, payload required"
 *          error, and stays alive. The driver proves liveness with a timer
 *          and exits cleanly.
 *
 ...
 *                      Names
 ***************************************************************************/
#define APP_NAME        "test_mqtt_" "malformed"
#define APP_DOC         "Self-contained MQTT broker malformed-frame regression test"
                ...
                {                                                   \n\
                    ...
                    'children': [                                   \n\
                        {                                           \n\
                            ...
                            'kw': {                                 \n\
                                'iamServer': false,                 \n\
                                'mqtt_client_id': 'malformed_client' \n\
                            },                                      \n\
                            'children': [                           \n\
                                {                                   \n\
                                    'name': 'mqtt_client',          \n\
