...
PUBLIC json_t *gobj_read_json_attr(hgobj gobj, const char *name); // WARNING it's not your own.
...
PUBLIC void gbuffer_remove(gbuffer gbuf); /* WARNING do not call gbuffer_remove(), call gbuffer_decref() */
PUBLIC void gbuffer_incref(gbuffer gbuf);
PUBLIC void gbuffer_decref(gbuffer gbuf);

/*
 *  READING
 */
PUBLIC void *gbuffer_cur_rd_pointer(gbuffer gbuf);  /* Return current reading pointer */
PUBLIC void gbuffer_reset_rd(gbuffer gbuf);        /* reset read pointer */

PUBLIC int gbuffer_set_rd_offset(gbuffer gbuf, size_t position);
PUBLIC int gbuffer_ungetc(gbuffer gbuf, char c);
PUBLIC size_t gbuffer_get_rd_offset(gbuffer gbuf);
/*
 * Pop 'len' bytes, return the pointer.
 * If there is no `len` bytes of data to pop, return 0, and no data is popped.
 */
PUBLIC void *gbuffer_get(gbuffer gbuf, size_t len);
PUBLIC char gbuffer_getchar(gbuffer gbuf);   /* pop one bytes */
PUBLIC size_t gbuffer_chunk(gbuffer gbuf);   /* return the chunk of data available */
PUBLIC char *gbuffer_getline(gbuffer gbuf, char separator);

/*
 *  WRITTING
 */
PUBLIC void *gbuffer_cur_wr_pointer(gbuffer gbuf);  /* Return current writing pointer */
PUBLIC void gbuffer_reset_wr(gbuffer gbuf);     /* reset write pointer (empty write/read data) */
PUBLIC int gbuffer_set_wr(gbuffer gbuf, size_t offset);
PUBLIC size_t gbuffer_append(gbuffer gbuf, void *data, size_t len);   /* return bytes written */
PUBLIC size_t gbuffer_append_string(gbuffer gbuf, const char *s);
PUBLIC size_t gbuffer_append_char(gbuffer gbuf, char c);              /* return bytes written */
PUBLIC int gbuffer_append_gbuf(gbuffer dst, gbuffer src);
PUBLIC int gbuffer_printf(gbuffer gbuf, const char *format, ...) JANSSON_ATTRS((format(printf, 2, 3)));
PUBLIC int gbuffer_vprintf(gbuffer gbuf, const char *format, va_list ap) JANSSON_ATTRS((format(printf, 2, 0)));;

/*
 *  Util
 */
PUBLIC void *gbuffer_head_pointer(gbuffer gbuf);  /* Return pointer to first position of data */
PUBLIC void gbuffer_clear(gbuffer gbuf);  // Reset write/read pointers

PUBLIC size_t gbuffer_leftbytes(gbuffer gbuf);   /* nº bytes remain of reading */
PUBLIC size_t gbuffer_totalbytes(gbuffer gbuf);  /* total written bytes */
PUBLIC size_t gbuffer_freebytes(gbuffer gbuf);   /* free space */
 ...
 *  Encode to base64
 */
PUBLIC gbuffer gbuffer_string_to_base64(const char* src, size_t len); // base64 without newlines
