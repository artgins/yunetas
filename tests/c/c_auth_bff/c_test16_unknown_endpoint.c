/****************************************************************************
 *          C_TEST16_UNKNOWN_ENDPOINT.C
 *
 *          Driver for test16_unknown_endpoint: sends POST /auth/nonexistent.
 *          The BFF does not recognise the URL and must reject with:
 *            - HTTP 404 response
 *            - body.error_code == "unknown_endpoint"
 *            - stats: kc_calls == 0, bff_errors == 1
 *
...
#include "c_test16_unknown_endpoint.h"
...
PRIVATE int ac_on_open(hgobj gobj, gobj_event_t event, json_t *kw, hgobj src)
{
    ...
    if(gobj_trace_level(gobj) & TRACE_MESSAGES) {
        gobj_trace_msg(gobj, "test16: connected to BFF, posting /auth/nonexistent (expect 404)");
    }

    /*
     *  POST to an unrecognised /auth/ path.
     *  The BFF falls through the endpoint dispatch and returns
...
PRIVATE int ac_on_message(hgobj gobj, gobj_event_t event, json_t *kw, hgobj src)
{
    ...
    if(gobj_trace_level(gobj) & TRACE_MESSAGES) {
        gobj_trace_msg(gobj, "test16: BFF response status=%d", status);
        if(jn_body) {
            gobj_trace_json(gobj, jn_body, "test16: BFF response body");
        }
    }
    ...
    if(status != 404) {
        gobj_log_error(gobj, 0,
            ...
            "msg",      "%s", "test16_unknown_endpoint: expected BFF 404, got different status",
            "expected", "%d", 404,
            "status",   "%d", status,
            NULL
        );
    ...
    if(!jn_body || !json_is_object(jn_body)) {
        gobj_log_error(gobj, 0,
            "function", "%s", __FUNCTION__,
            "msgset",   "%s", MSGSET_APP_ERROR,
            "msg",      "%s", "test16_unknown_endpoint: BFF error body missing or not an object",
            NULL
        );
        goto end;
    }
    if(json_is_true(json_object_get(jn_body, "success"))) {
        gobj_log_error(gobj, 0,
            "function", "%s", __FUNCTION__,
            "msgset",   "%s", MSGSET_APP_ERROR,
            "msg",      "%s", "test16_unknown_endpoint: body.success must be false",
            NULL
        );
        goto end;
    }
    const char *got_code = kw_get_str(gobj, jn_body, "error_code", "", 0);
    if(strcmp(got_code, "unknown_endpoint") != 0) {
        gobj_log_error(gobj, 0,
            "function", "%s", __FUNCTION__,
            "msgset",   "%s", MSGSET_APP_ERROR,
            "msg",      "%s", "test16_unknown_endpoint: body.error_code mismatch",
            "expected", "%s", "unknown_endpoint",
            "got",      "%s", got_code,
            NULL
        );
        goto end;
    ...
    if(empty_string(got_error)) {
        gobj_log_error(gobj, 0,
            "function", "%s", __FUNCTION__,
            "msgset",   "%s", MSGSET_APP_ERROR,
            "msg",      "%s", "test16_unknown_endpoint: body.error fallback empty",
            NULL
        );
        goto end;
    }
    ...
    test_helpers_check_stats(gobj, bff, "test16_unknown_endpoint", expected);
...
PRIVATE int ac_on_close(hgobj gobj, gobj_event_t event, json_t *kw, hgobj src)
{
    ...
    if(!priv->test_passed) {
        gobj_log_error(gobj, 0,
            "function", "%s", __FUNCTION__,
            "msgset",   "%s", MSGSET_APP_ERROR,
            "msg",      "%s", "test16_unknown_endpoint: connection closed before success",
            NULL
        );
        TEST_HELPERS_BEGIN_DYING(priv);
    }
...
GOBJ_DEFINE_GCLASS(C_TEST16_UNKNOWN_ENDPOINT);
...
{
    return create_gclass(C_TEST16_UNKNOWN_ENDPOINT);
}
