...
PRIVATE json_t *cmd_truncate_log_file(hgobj gobj, const char *cmd, json_t *kw, hgobj src);
...
SDATACM2(DTP_SCHEMA,    "get-global-no-trace",      SDF_AUTHZ_X, 0,      0, cmd_get_global_no_trace,"Get global no-trace levels"),
SDATACM2(DTP_SCHEMA,    "set-global-no-trace",      SDF_AUTHZ_X, 0,      pm_set_global_tr,cmd_set_global_no_trace,"Set global no-trace level"),
...
SDATA (DTP_BOOLEAN, "enable_subscription_authz", SDF_RD, "0",          "Enforce the authorization of an external subscription to an EVF_AUTHZ_SUBSCRIBE event (needs a C_AUTHZ role model). Default off keeps historical behaviour."),
 ...
 *  Show register
 ***************************************************************************/
static const json_desc_t services_desc[] = { // HACK must match with gobj_services()
// Name             Type        Defaults    Fillspace
{"service",         "string",   "",         "40"},  // First item is the pkey
{"gclass",          "string",   "",         "20"},
{"top-service",     "boolean",  "",         "11"},
{"priority",        "int",      "",         "10"},
{"cmds",            "boolean",  "",         "10"},
{0}
};
...
PRIVATE json_t *cmd_write_attr(hgobj gobj, const char *cmd, json_t *kw, hgobj src)
{
    ...
    if(empty_string(gobj_name_)) {
        json_t *kw_response = build_command_response(
            gobj,
            -1,     // result
            json_sprintf("%s: what gobj?", gobj_yuno_role_plus_name()),
            0,      // jn_schema
            0       // jn_data
        );
        JSON_DECREF(kw)
    ...
    if(empty_string(attribute)) {
        json_t *kw_response = build_command_response(
            gobj,
            -1,     // result
            json_sprintf("%s: what attribute?", gobj_yuno_role_plus_name()),
            0,      // jn_schema
            0       // jn_data
        );
        JSON_DECREF(kw)
    ...
    if(!svalue) {
        json_t *kw_response = build_command_response(
            gobj,
            -1,     // result
            json_sprintf("%s: what value?", gobj_yuno_role_plus_name()),
            0,      // jn_schema
            0       // jn_data
        );
        JSON_DECREF(kw)
...
PRIVATE json_t *cmd_authzs(hgobj gobj, const char *cmd, json_t *kw, hgobj src)
{
    json_t *kw_response = build_command_response(
        gobj,
        -1,     // result
        json_sprintf("%s: Not yet implemented", gobj_yuno_role_plus_name()),
        0,      // jn_schema
        0       // jn_data
    );
    JSON_DECREF(kw)
...
PRIVATE json_t *cmd_view_gclass(hgobj gobj, const char *cmd, json_t *kw, hgobj src)
{
    ...
    if(empty_string(gclass_name_)) {
        json_t *kw_response = build_command_response(
            gobj,
            -1,     // result
            json_sprintf("%s: what gclass?", gobj_yuno_role_plus_name()),
            0,      // jn_schema
            0       // jn_data
        );
        JSON_DECREF(kw)
...
PRIVATE json_t *cmd_set_trace_filter(hgobj gobj, const char *cmd, json_t *kw, hgobj src)
{
    ...
    if(empty_string(trace_value)) {
        json_t *kw_response = build_command_response(
            ...
            -1,     // result
            json_sprintf(
                "%s: set or re-set?", gobj_yuno_role_plus_name()
            ),
            0,      // jn_schema
            0       // jn_data
        );
    ...
    if(empty_string(attr) && (set || !all)) {
        json_t *kw_response = build_command_response(
            gobj,
            -1,
            json_sprintf("%s: what attr?", gobj_yuno_role_plus_name()),
            0,
            0
        );
        JSON_DECREF(kw)
...
PRIVATE json_t *cmd_reset_all_traces(hgobj gobj, const char *cmd, json_t *kw, hgobj src)
{
    ...
    json_t *kw_response = build_command_response(
        gobj,
        -1,
        json_sprintf("%s: What gclass or gobj?", gobj_yuno_role_plus_name()),
        0,
        0
    );
    JSON_DECREF(kw)
...
PRIVATE json_t* cmd_set_deep_trace(hgobj gobj, const char* cmd, json_t* kw, hgobj src)
{
    ...
    json_t *kw_response = build_command_response(
        ...
        0,
        json_sprintf(
            "%s: daemon debug set to %d", gobj_yuno_role_plus_name(), trace
        ),
        0,
        0
    );
...
PRIVATE json_t *cmd_truncate_log_file(hgobj gobj, const char *cmd, json_t *kw, hgobj src)
{
    rotatory_truncate(0); // WARNING truncate all files
    json_t *kw_response = build_command_response(
        gobj,
        0,
        json_sprintf("%s: Truncate current log done.", gobj_yuno_role_plus_name()),
        0,
        0
    );
    JSON_DECREF(kw)
...
PRIVATE json_t *cmd_reset_log_counters(hgobj gobj, const char *cmd, json_t *kw, hgobj src)
{
    ...
    json_t *kw_response = build_command_response(
        gobj,
        0,
        json_sprintf("%s: Log counters reset.", gobj_yuno_role_plus_name()),
        0,
        0
    );
    JSON_DECREF(kw)
...
PRIVATE json_t *cmd_add_log_handler(hgobj gobj, const char* cmd, json_t* kw, hgobj src)
{
    ...
    if(empty_string(handler_name)) {
        json_t *kw_response = build_command_response(
            gobj,
            -1,
            json_sprintf("%s: What name?", gobj_yuno_role_plus_name()),
            0,
            0
        );
        JSON_DECREF(kw)
    ...
    if(strcmp(handler_type, "file")==0) {
        ...
        if(empty_string(url)) {
            json_t *kw_response = build_command_response(
                gobj,
                -1,
                json_sprintf("%s: What url?", gobj_yuno_role_plus_name()),
                0,
                0
            );
            JSON_DECREF(kw)
    ...
    json_t *kw_response = build_command_response(
        gobj,
        added>0?0:-1,
        json_sprintf("%s: %d handlers added.", gobj_yuno_role_plus_name(), added),
        0,
        0
    );
    JSON_DECREF(kw)
...
PRIVATE json_t *cmd_del_log_handler(hgobj gobj, const char* cmd, json_t* kw, hgobj src)
{
    ...
    json_t *kw_response = build_command_response(
        gobj,
        deletions>0?0:-1,
        json_sprintf("%s: %d handlers deleted.", gobj_yuno_role_plus_name(), deletions),
        0,
        0
    );
    JSON_DECREF(kw)
...
PRIVATE json_t* cmd_add_allowed_ip(hgobj gobj, const char* cmd, json_t* kw, hgobj src)
{
    ...
    if(empty_string(ip)) {
        ...
        return build_command_response(
            gobj,
            -1,     // result
            json_sprintf("%s: What ip?", gobj_yuno_role_plus_name()),
            0,      // jn_schema
            0       // jn_data
        );
    }
    if(!kw_has_key(kw, "allowed")) {
        ...
        return build_command_response(
            gobj,
            -1,     // result
            json_sprintf("%s: Allowed, TRUE or FALSE?", gobj_yuno_role_plus_name()),
            0,      // jn_schema
            0       // jn_data
        );
    }
...
PRIVATE json_t* cmd_add_denied_ip(hgobj gobj, const char* cmd, json_t* kw, hgobj src)
{
    ...
    if(!kw_has_key(kw, "denied")) {
        ...
        return build_command_response(
            gobj,
            -1,     // result
            json_sprintf("%s: Denied, TRUE or FALSE?", gobj_yuno_role_plus_name()),
            0,      // jn_schema
            0       // jn_data
        );
    }
    ...
    if(!gobj2view) {
        ...
        if(!gobj2view) {
            json_t *kw_response = build_command_response(
                ...
                -1,
                json_sprintf(
                    "%s: gobj '%s' not found.", gobj_yuno_role_plus_name(), gobj_name_
                ),
                0,
                0
            );
    ...
    if(!empty_string(event_name)) {
        ...
        if(event_type) {
            ...
            json_t *kw_response = build_command_response(
                ...
                -1,
                json_sprintf(
                    "%s: event '%s' not found.", gobj_yuno_role_plus_name(), event_name
                ),
                0,
                NULL
            );
