 ...
 *  ATTR: read
 ***************************************************************************/
PUBLIC json_t *gobj_read_json_attr(hgobj gobj_, const char *name)
{
    gobj_t *gobj = gobj_;
    ...
    size_t max_memory_size)
{
     ...
     *   Alloc memory
     *---------------------------------*/
    gbuffer_t *gbuf = sys_malloc_fn(sizeof(*gbuf));
    if(!gbuf) {
        gobj_log_error(0, LOG_OPT_TRACE_STACK,
            "function",     "%s", __FUNCTION__,
            "msgset",       "%s", MSGSET_MEMORY_ERROR,
    ...
    gbuf->data = sys_malloc_fn(data_size+1);
    if(!gbuf->data) {
        gobj_log_error(0, LOG_OPT_TRACE_STACK,
            "function",     "%s", __FUNCTION__,
            "msgset",       "%s", MSGSET_MEMORY_ERROR,
...
PRIVATE BOOL gbuffer_realloc(gbuffer_t *gbuf, size_t need_size)
{
     ...
     *  Realloc buffer
     */
    new_buf = sys_realloc_fn(gbuf->data, more+1);
    if(!new_buf) {
        gobj_log_error(0, LOG_OPT_TRACE_STACK,
            "function",     "%s", __FUNCTION__,
            "msgset",       "%s", MSGSET_INTERNAL_ERROR,
